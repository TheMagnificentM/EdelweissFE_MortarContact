...
Slave-Knoten. Sie ist die \emph{gemittelte Knotennormale} von \citet[Gl.~(37)]{popp2010}, in
derselben Form bei \citet[Gl.~(4.41)]{farah}: die Normale der angrenzenden Facette wird an der
Naturkoordinate $\bm\xi_j$ \emph{dieses Knotens} ausgewertet, über die angrenzenden Facetten
summiert und erst danach normiert (\code{compute\_normals}):
\begin{equation}
\bm n_j = \frac{\sum_{f\ni j}\bm n_{j,f}}{\big\|\sum_{f\ni j}\bm n_{j,f}\big\|},
\qquad
\bm n_{j,f} =
...
\paragraph{Die Gewichtung ist eine offene Wahl -- auch in der Literatur.}
Summiert wird oben die \emph{nicht} normierte Facettennormale; ihre Länge ist die lokale
Flächen-Jacobideterminante, sodass eine große Facette einen gemeinsamen Knoten stärker zieht als
eine kleine. Das ist eine von mehreren dokumentierten Möglichkeiten, und die Quellen sind sich
darin uneinig:
\begin{itemize}\itemsep2pt
  \item \citet[Gl.~(37)]{popp2010} summieren die \emph{Einheits}normalen, also ungewichtet. Sie
        halten dazu ausdrücklich fest, das geschehe \emph{``for simplicity''}, die gewichtete
        Erweiterung nach \citet{yangmeng2005} sei \emph{``straightforward and has been tested as
        well, but does not change the results considerably''}.
  ...
  \item Der Code gewichtet mit der lokalen Jacobideterminante, also mit der eigenen Facettengröße.
\end{itemize}
Auf einem gleichmäßigen Netz fallen alle drei zusammen; die Testreihe kann sie daher nicht
unterscheiden. Der \emph{entscheidende} Punkt -- die Auswertung an $\bm\xi_j$ statt je Facette --
ist von der Gewichtung unberührt und in allen drei Quellen derselbe.
...
in keinem der beiden Läufe, obwohl einzelne Inkrementversuche $23$ (hex8) bzw.\ $25$
Newton-Iterationen (hex20) brauchen -- letzteres die Iterationsgrenze des Lösers selbst. Das Set
...
und in welchen Increments. Und da dieselbe Rechnung $12$ Cutbacks enthält, ist nach der oben
...
\code{contact\_l\_head\_vert} an fünf Slave-Knoten, im ungünstigsten Fall bei $-0{,}85$ des größten
Gewichts, und das bereits ab Increment~1. Der Sliver-Rückfall spricht dabei \emph{nicht} an: es ist
...
derselbe Knoten bei voller Überdeckung trüge. Der Umfang passt dazu: $5$ der $43$ Slave-Knoten
\emph{eines} der sechs Kontaktpaare, bei $498$ Multiplikatorzeilen im Modell, also rund $1\,\%$.
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Knotennormalen gegen eine unabhängige Referenz.} Auf einem Zylinderausschnitt
        ...
        Geprüft wird nicht bloß, dass der Fehler fällt, sondern eine Mindest\emph{ordnung}
        ($\ge1{,}8$ linear, $\ge3{,}5$ quadratisch). Das ist der eigentliche Regressionsschutz:
        eine facettenweise gebildete Normale erfüllt jede blosse Reduktion ebenfalls, fällt aber
        auf Faktor $2{,}00$ bei rund dem $250$-fachen Fehler zurück -- nachgestellt und als
        \emph{fehlschlagend} verifiziert.
...
dieselbe Gesamt-Auflagerbettung. Die quadratische Rechnung läuft bis zur vorgegebenen
Endverschiebung von $1{,}1$\,mm durch; das Kreuz markiert den Abbruch der linearen Rechnung.
Erzeugt mit \file{POT\_Dejori/plot\_comparison.py}.}
\label{fig:pot}
\end{figure}
...
\midrule
hex8 (\code{GC3D8}/\code{CONQUAD4})    & $199{,}1$\,kN/mm & $22{,}50$\,kN & $0{,}143$\,mm & $0{,}815$\,mm \\
hex20 (\code{GC3D20R}/\code{CONQUAD8}) & $197{,}6$\,kN/mm & $19{,}77$\,kN & $0{,}121$\,mm & $1{,}100$\,mm \\
\midrule
...
Die Sekantensteifigkeiten stimmen auf $0{,}8\,\%$ überein. Das ist das erwartete Ergebnis: im
...
Am Peak trennen sich die Kurven. Die quadratische Rechnung trifft mit $19{,}77$\,kN die Oberkante
...
ein, und die Kurve läuft auf einen höheren, späteren Peak.

\paragraph{Entfestigender Ast.}
Nach dem Peak liegt die lineare Rechnung durchgehend über der quadratischen: sie verläuft auf
$95\,\%$ ihres Nachbruchbereichs innerhalb des Streubands, die quadratische auf $24\,\%$. Bei
einem lokalisierenden Material ist die bessere Übereinstimmung der linearen Rechnung kein
Qualitätsmerkmal, sondern Ausdruck der gröberen Auflösung der Schädigungszone. Die quadratische
Kurve fällt steiler ab, unterschreitet das Streuband zwischen $u\approx0{,}3$ und $0{,}9$\,mm und
kehrt ab $u=0{,}89$\,mm wieder in das Band zurück; bis zur vorgegebenen Endverschiebung läuft sie
auf ein Restkraftplateau von $3{,}1$\,kN aus, das sind $16\,\%$ der Traglast. Der zu tiefe Verlauf
dazwischen ist derselbe Befund wie am Peak, nur mit umgekehrtem Vorzeichen: die feiner aufgelöste
Schädigungszone entfestigt schneller.
...
Die lineare Rechnung erreicht die vorgegebene Endverschiebung nicht. Sie läuft bis
$u=0{,}815$\,mm, also weit über den Peak hinaus, und bricht dort im entfestigenden Ast ab: die
Kurve endet mit \emph{senkrechter Tangente} ($\mathrm dF/\mathrm du\to-\infty$; die
Sekantensteifigkeit fällt über die letzten Inkremente von $-27$ auf $-166$\,kN/mm, während $u$
praktisch stehenbleibt). Der Beton kann der aufgezwungenen Verschiebung nicht mehr
folgen -- das Material versagt. Der Löser meldet entsprechend, dass die Schrittweite nicht weiter
reduziert werden kann (\code{Cannot reduce increment size}), und beendet den Step. Die Kurve ist
damit inhaltlich vollständig: Anstieg, Traglast und ein großer Teil des Nachbruchbereichs sind
erfasst. Aufwand: $131$ Inkremente, $12$ Cutbacks, $1\,009$ Newton-Iterationen, rund $21$ Minuten
Rechenzeit.

\paragraph{hex20: vollständiger Durchlauf.}
Die quadratische Rechnung erreicht die vorgegebenen $1{,}1$\,mm und endet regulär mit dem
Laststep. Der Aufwand dafür ist erheblich: $1\,176$ Inkremente, $46$ Cutbacks, $8\,459$
Newton-Iterationen, $12{,}2$ Stunden ($43\,858$\,s). Er ist zudem stark auf den Bereich um die
Traglast konzentriert -- $7\,350$ der $8\,459$ Iterationen, also $87\,\%$, entfallen auf die
ersten $74\,\%$ des Weges (bis zu jenem $u=0{,}815$\,mm, an dem die lineare Rechnung abbricht).
Der auslaufende Ast danach ist vergleichsweise billig.

\paragraph{Einordnung.}
Auf diesem gemeinsamen Wegstück lassen sich beide Rechnungen direkt vergleichen, und der Aufwand
zerfällt in zwei Faktoren: die quadratische Rechnung braucht das $7{,}3$-fache an
Newton-Iterationen ($7\,350$ gegenüber $1\,009$), und jede einzelne Iteration kostet mit $5{,}2$
statt $1{,}25$\,s das $4{,}2$-fache. Der zweite Faktor ist Netzgröße ($3{,}5$-mal so viele Knoten);
der erste ist es nicht -- dieselbe Last, dieselbe Geometrie, dieselben sechs Kontaktpaare, nur mehr
Newton-Schritte je Wegstück. Dafür kommen zwei Ursachen in Frage, die diese Rechnung nicht trennt:
die feiner aufgelöste und damit steilere Lokalisierung des Materialmodells, und die fehlende
konsistente Linearisierung des Kontaktbeitrags (Abschnitt~\ref{sec:frozen}) -- ohne die
geometrischen Ableitungen ist der Konvergenzradius kleiner, und das Verfahren greift häufiger zu
Cutbacks ($46$ gegenüber $12$). Hier läge der Gewinn einer konsistenten Linearisierung
...
\begin{enumerate}\itemsep2pt
  \item die Knotennormale $\bm n_j$ nach \eqref{eq:nodalnormal} -- die Ableitung einer normierten
        Summe von Kreuzprodukten. Für diesen einen Punkt ist die Herleitung bereits vorhanden und
        unmittelbar übertragbar: \citet[Gl.~(A1)--(A2)]{popp2010} linearisieren genau
        \eqref{eq:nodalnormal}. Weil dort die \emph{Einheits}normalen der Nachbarfacetten summiert
        werden, hier dagegen die mit der Jacobideterminante gewichteten (siehe
        Abschnitt~\ref{sec:knotennormale}), entfällt für den Code sogar die innere
        Normierungsableitung (A2), und es bleibt $\delta\hat{\bm n}_j=\sum_f\delta\bm n_{j,f}$;
