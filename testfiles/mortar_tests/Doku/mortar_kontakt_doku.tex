...
$\bm D,\bm M$ in \code{compute\_mortar\_coupling\_matrices} assembliert (die Slave--Master-Matrix
heißt dort \code{C}; vgl.\ Notationskonvention).
...
\file{hertz\_hex20\_medium} (CONQUAD8) entfallen $326$ der $599$ Einträge von $\bm D$, also
$54{,}4\,\%$, auf Nebendiagonalen, mit
$\max|\tilde D_{jk}|/\max|\tilde D_{jj}| = \tfrac13$ -- gerade der Wert $\alpha$ der
Basistransformation. Die im Code verwendete Größe $\tilde D_{jj}$ ist konsistent dazu die
\emph{Zeilensumme} $\sum_k \bm D[j,k]$ und nicht das Diagonalelement; beide unterscheiden sich auf
diesem Modell um den Faktor $1{,}6$.
...
naheliegt, steht in Abschnitt~\ref{sec:sgnd}.
...
verschwindet ohnehin entweder $\tilde g_j$ oder $\lambda_{n,j}$; die Struktur der NCP und der
...
\paragraph{Voreinstellung im Code.} Wird \code{cn} in der Eingabedatei nicht angegeben, so leitet der
Constraint den Wert selbst ab: als kleinsten \emph{Anfangs}-Elastizitätsmodul der Materialien, deren
Elemente einen Knoten mit einer der beiden Kontaktflächen teilen
(\file{mortarcontact.py}, Methode \code{\_resolve\_c\_n}). Drei Punkte dazu:
...
\begin{itemize}
  ...
  \item \textbf{Warum der Wert gemeldet wird.} Dass die erste Materialkonstante der
        Elastizitätsmodul ist, ist Marmot-Konvention und keine Garantie. Der abgeleitete Wert wird
        deshalb über die Laufzeitdiagnosen ausgegeben und bleibt damit prüfbar. Eine explizite
        Angabe von \code{cn} hat weiter Vorrang und löst keine Meldung aus.
\end{itemize}

\noindent Eine dimensionsbehaftete Zahlenkonstante als Voreinstellung wäre nur für ein einziges
Einheitensystem richtig; die Ableitung ersetzt sie.
...
$\bm N_m$ unmittelbar in $\bm D$ und $\bm C$ ein. Keine nachgelagerte Stufe kann ihn bemerken.
...
Newton-Iteration und die Segmentquadratur ergibt das relativ $\sim10^{-12}$ in $\bm D$ und $\bm C$
...
letzte Iterationswert geht über $\bm N_s$, $\bm N_m$ direkt in $\bm D$ und $\bm C$ ein und ist von
...
Slave-Knoten. Diese Increments haben dort den falschen Zweig erzwungen; die Signorini-Bedingungen
sind für sie nicht strikt erfüllt. Das ist keine Randerscheinung, sondern genau der Befund, den eine
Globalisierung des semiglatten Newton-Verfahrens adressieren würde
(Abschnitt~\ref{sec:ausblick:lin}). Weder Sliver-Rückfall noch negatives Knotengewicht noch eine
...
\code{cn=\path{<float>}} & Optional (Standard $0$, d.\,h.\ \emph{ableiten}). Komplementaritätsparameter $c_n$ der Normal-NCP (Abschnitt~\ref{sec:ncp}). Ohne Angabe wird der kleinste Anfangs-Elastizitätsmodul der an die Kontaktflächen angrenzenden Materialien angesetzt und der verwendete Wert gemeldet. Eine explizite Angabe hat Vorrang und sollte in der Größenordnung $\mathcal O(E)$ des weicheren Kontaktpartners liegen. \\
\bottomrule
\end{tabularx}
