...
$\bm n_j$ & \code{normals} & gemittelte Knotennormale des Slave-Knotens $j$ & \eqref{eq:nodalnormal} \\
...
$\tilde D_{jj}$ & \code{current\_D\_rowsum} & Knotengewicht $\int_{\gamma_c^{\text{ovl}}}\tilde N_j$, vorzeichenbehaftet & \eqref{eq:weightisprimal} \\
...
Schwerpunkt der Sub-Zelle und Normale ihre \emph{eigene}, aus den \emph{Eck}knoten gebildete
Facettennormale
\begin{equation}
...
\end{equation}
die Dreiecksform für dreieckige, die Quadform (Diagonalenkreuzprodukt) für viereckige Sub-Zellen.
Die Hilfsebene ist damit
\begin{equation}
...
\label{eq:auxplane}
\end{equation}
Da die Sub-Zellen der Segmentierung ohnehin \emph{linear} sind
(Abschnitt~\ref{sec:subcells}), ist $\bm n_f$ dort die exakte Normale ihrer Ebene und keine
Näherung -- anders als sie es wäre, wenn man sie auf die gekrümmte Facette als Ganzes anwendete.

Die Formulierung benutzt damit \textbf{zwei verschiedene Normalendefinitionen}: $\bm n_c$ je
Sub-Zelle für die \emph{Segmentierung} und die gemittelte Knotennormale $\bm n_j$
\eqref{eq:nodalnormal} (Abschnitt~\ref{sec:knotennormale}) für die \emph{Kinematik}, also für den
gewichteten Spalt und die Kraftrichtung. Das ist kein Versehen und in der Literatur üblich -- die Wahl der Hilfsebene ist
...
$\sum_k \bm D[j,k]$ und nicht das Diagonalelement. Die beiden fallen an den Eckknoten zusammen und
unterscheiden sich an den Kantenmittelknoten um den Faktor~$3$ -- gemessen an
\file{hertz\_hex20\_medium} liegt $\tilde D_{jj}/D_{jj}$ genau zwischen $\tfrac13$ und $1$, mit
dem Median $\tfrac13$. Das ist keine Zufälligkeit des Modells, sondern folgt aus
\eqref{eq:DTfaktor}: mit $D_{jj}=\tilde D_{jj}(\bm T_e^{-1})_{jj}$ und
$(\bm T_e^{-1})_{jj}=1$ (Ecke) bzw.\ $(1-2\alpha)^{-1}=3$ (Mittelknoten) für
$\alpha=\tfrac13$. Dass die Zeilensumme die richtige Größe ist, folgt aus der Partition of Unity: mit
$\sum_k N_k^{(1)}=1$ ist
\begin{equation}
...
er summiert über alle Einträge der Zeile~$j$ mit $|D_{jk}|>10^{-12}\max_{ab}|D_{ab}|$
(\code{\_nonzero\_rows}, Parameter \code{rtol}).
...
$\tfrac{7}{18}$ bzw.\ $\tfrac29$ (nachgerechnet in \file{07\_patch\_test\_2d}). Dass das beim
CONLINE3 aufgeht, beim CONQUAD8 dagegen nicht, ist kein Zufall, sondern eine Frage der Schwelle:
aus $\tilde N_{\text{Ecke}}=(\tfrac12-\alpha)(\xi-1)\big(\xi-\tfrac{\alpha}{1/2-\alpha}\big)$ folgt
für CONLINE3 punktweise Nichtnegativität ab $\alpha\ge\tfrac14$, für CONQUAD8 dagegen erst ab
$\alpha\ge\tfrac38$ \eqref{eq:alphastar} -- der implementierte Wert $\tfrac13$ liegt zwischen
beiden. Genau diese Absicherung fehlt beim CONQUAD9 -- der folgende Abschnitt begründet, warum,
und benennt die Grenze dieser Begründung.
...
\end{equation}
eine strikt stärkere Forderung als \eqref{eq:intpos}. Für die Serendipity-Eckfunktion lässt sich die
zugehörige Schwelle geschlossen angeben. Auf der Diagonalen $\xi=\eta=t$ des Referenzquadrats ist
mit $\tilde N_1 = N_1+\alpha(N_5+N_8)$
\begin{equation}
\tilde N_{\text{Ecke}}(t,t) \;=\; (1-t)^2\Big[\big(\alpha-\tfrac12\big)\,t + \big(\alpha-\tfrac14\big)\Big],
\label{eq:quad8diag}
\end{equation}
und auf dieser Diagonalen liegt auch das Minimum über das gesamte Referenzquadrat (nachgerechnet
auf einem $601\times601$-Gitter). Der Vorfaktor $(1-t)^2$ ist nichtnegativ, die eckige Klammer fällt
für $\alpha<\tfrac12$ linear in $t$, ihr kleinster Wert auf $t\in[-1,1]$ ist also $2\alpha-\tfrac34$.
Damit gilt
\begin{equation}
\tilde N_{\text{Ecke}}\;\ge\;0 \ \text{ auf dem ganzen Element}
\quad\Longleftrightarrow\quad \alpha\;\ge\;\alpha^\ast=\tfrac38 .
\label{eq:alphastar}
\end{equation}
Die Schwelle liegt damit \emph{oberhalb} beider Literaturwerte: \textbf{weder $\alpha=\tfrac15$ noch
$\alpha=\tfrac13$ macht die transformierte Eckfunktion punktweise nichtnegativ.} Das ist kein
Widerspruch zur Literatur -- \citet{popp2012} und \citet{farah} begründen ihre Werte ausdrücklich
mit der Integral-Positivität \eqref{eq:intpos} und führen die punktweise Nichtnegativität nur für
die \emph{ordnungsreduzierte} (bi-)lineare Multiplikatorwahl an
(\citealp[S.~B429]{popp2012}), nicht für die transformierte quadratische Basis.

Was $\alpha$ steuert, ist die \emph{Ausdehnung} des Gebiets, auf dem die Eckfunktion negativ wird:
nach \eqref{eq:quad8diag} ist sie negativ für $t>(\alpha-\tfrac14)/(\tfrac12-\alpha)$, also ab
$t>-\tfrac16$ bei $\alpha=\tfrac15$ und erst ab $t>\tfrac12$ bei $\alpha=\tfrac13$. Flächenmäßig
nimmt das Negativgebiet damit $65\,\%$ des Referenzquadrats ein (ausgezählt auf einem Gitter)
gegenüber exakt $12{,}5\,\%$ bei $\alpha=\tfrac13$: dort ist
$\tilde N_{\text{Ecke}}=\tfrac{1}{12}(1-\xi)(1-\eta)\,(1-\xi-\eta)$, also negativ genau auf dem
Dreieck $(1-\xi)+(1-\eta)<1$ der Fläche $\tfrac12$ von $4$. Auch die Tiefe unterscheidet sich um mehr als eine
Größenordnung -- die Minima sind $-\tfrac{343}{4860}\approx-7{,}06\cdot10^{-2}$ bei $t=\tfrac29$
gegenüber $-\tfrac{1}{324}\approx-3{,}09\cdot10^{-3}$ bei $t=\tfrac23$. Ein knotenweises Gewicht
$\int_{\gamma_c^{\text{ovl}}}\tilde N_j$ wird deshalb erst dann negativ, wenn die Überlappung
überwiegend in diesem Restgebiet liegt; eine bloße Teilüberdeckung genügt nicht.

\paragraph{Numerischer Befund.}
Dies ist an den Geometrien der Tests~05 und~08 (Abschnitt~\ref{sec:verif}) gemessen. Die
Testreihe selbst rechnet ausschließlich mit $\alpha=\tfrac13$ -- der Wert steht als Konstante in
\code{element.getBasisTransformation} und ist kein Testparameter --; die
$\alpha=\tfrac15$-Vergleichswerte stammen aus denselben Rechnungen mit vorübergehend
umgestelltem $\bm T_e$ und sind daher durch einen Lauf der Testreihe nicht reproduzierbar,
wohl aber durch dieselbe Umstellung:
\begin{itemize}\itemsep2pt
  \item \textbf{Test~05 (partielle Überdeckung, CONQUAD8):} Bei um $0.3$ verschobenem Master
        ...
        mit $\alpha=\tfrac15$ wird ein Eckknoten-Gewicht \emph{negativ} ($\approx-0.0049$). Der
        Unterschied ist nach \eqref{eq:quad8diag} zu erwarten: bei $\alpha=\tfrac15$ reicht das
        Negativgebiet weit in die Überlappung hinein, bei $\alpha=\tfrac13$ wird sein Beitrag vom
        positiven Anteil der Überlappung überwogen.
  \item \textbf{Grenze auch bei $\alpha=\tfrac13$ (CONQUAD8, konsistenter Pfad):} Legt man die
        Überlappung gezielt in das verbleibende Negativgebiet -- Slave $[0,1]^2$, Master so
        verschoben, dass nur die Ecke $[s,1]^2$ überdeckt ist --, so wird das Gewicht des
        gegenüberliegenden Eckknotens ab $s>\tfrac58$, also unterhalb von $14{,}06\,\%$
        Überdeckung, negativ: bei $9\,\%$ Überdeckung $-1{,}35\cdot10^{-4}$, das sind
        $-0{,}26\,\%$ des größten Gewichts. Der Sliver-Rückfall (Abschnitt~\ref{sec:sliver})
        spricht dabei \emph{nicht} an; das Gewicht wird auf dem konsistenten Weg negativ. Für
        diese Konfiguration ist das Gewicht geschlossen angebbar,
        $\tilde D_{jj}=-\tfrac{1}{36}(s-1)^4(8s-5)$ mit Nullstelle bei $s=\tfrac58$; die
        Segmentquadratur trifft sie auf $10^{-14}$ absolut
        (\file{05\_quadratic\_segmentation}, \code{test\_partial\_coverage\_corner\_can\_turn\_weight\_negative}).
        Die Konfiguration ist auch nicht konstruiert -- im Ausziehversuch tritt sie auf
...
\end{itemize}
Der Wert $\alpha=\tfrac13$ sichert \eqref{eq:pointwisepos} demnach \emph{nicht}; er verkleinert das
Negativgebiet. Gegenüber $\alpha=\tfrac15$ schrumpft es auf der Diagonalen von $t>-\tfrac16$ auf
$t>\tfrac12$ und wird um den Faktor $23$ flacher, und das genügt, damit die Gewichte bei jeder
Überdeckung von nennenswerter Größe positiv bleiben -- zusammen mit dem in Test~08 gemessenen
Genauigkeitsvorteil bei Verzerrung ist das die Begründung der Wahl. Der Preis ist der Verzicht auf
die exakte Reproduktion linearer Polynome, die $\alpha=\tfrac15$ auf unverzerrten Elementen böte;
für die Konsistenzordnung des Kontakts ist dies unkritisch, und die unverzerrten Patch-Tests werden
mit beiden Werten exakt bestanden. Vollständig ausräumen ließe sich \eqref{eq:pointwisepos} erst
mit $\alpha\ge\tfrac38$ nach \eqref{eq:alphastar}. Das ist \emph{nicht} umgesetzt und hier auch
nicht erprobt: $\tfrac38$ entfernt sich weiter von Popps flächenspezifischem $\tfrac15$, und welche
Auswirkung das auf die Genauigkeit bei verzerrten Netzen hätte, ist nicht gemessen.

\begin{quote}\small
\textbf{Bemerkung.} Da im Zielanwendungsfall (Ausziehversuch eines Ankers im Betonblock)
verzerrte Netze und über den Lastverlauf wechselnder Teilkontakt auftreten, ist die Robustheit der
knotenweisen Gewichte hier die ausschlaggebende Eigenschaft; sie hat Vorrang vor der exakten
Linearreproduktion von $\alpha=\tfrac15$. Dies deckt sich mit Farahs Begründung ``numerische
Erfahrung'' (\citealp[Gl.~(6.20)]{farah}). Verbessert ist diese Robustheit damit, garantiert ist
sie nicht: den Rest trägt der $\mathrm{sgn}(\tilde D_{jj})$-Zweig des Aktivierungstests
(Abschnitt~\ref{sec:sgnd}) -- und der Ausziehversuch zeigt, dass dieser Zweig dort tatsächlich
gebraucht wird.
\end{quote}
...
Der Unterschied zum CONQUAD8 ist gradueller, nicht grundsätzlicher Natur. Auch dessen
transformierte Eckfunktion ist bei $\alpha=\tfrac13$ nicht punktweise nichtnegativ
(Abschnitt~\ref{sec:alphachoice}), ihr Minimum liegt aber bei $-\tfrac{1}{324}$ statt $-\tfrac18$
-- ein Faktor $40$ --, und das Negativgebiet bedeckt exakt $12{,}5\,\%$ des Referenzquadrats statt
$49{,}8\,\%$. Beim CONQUAD8 muss die
Überdeckung deshalb nahezu vollständig in dieses Restgebiet fallen, damit ein Gewicht kippt; beim
CONQUAD9 genügt bereits eine freiliegende Kante. Eine Absicherung im Sinne von
\eqref{eq:pointwisepos} hat dieser Code für keinen der beiden Typen.
...
machen -- bei CONQUAD8 wäre dafür $\alpha\ge\tfrac38$ nötig \eqref{eq:alphastar}, umgesetzt ist
$\tfrac13$ -- oder das Vorzeichen konsistent behandeln (Abschnitt~\ref{sec:sgnd}). Bei CONLINE3
greift der erste Ausweg ($\alpha^\ast=\tfrac14$), bei CONQUAD8 und CONQUAD9 derzeit nur der zweite.
...
\subsection{Gemittelte Knotennormale}
\label{sec:knotennormale}
Der gewichtete Spalt \eqref{eq:wgap_pos} und die Projektion benötigen eine eindeutige Normale pro
Slave-Knoten. Sie ist die \emph{gemittelte Knotennormale} von \citet[Gl.~(37)]{popp2010}: die
Normale der angrenzenden Facette wird an der Naturkoordinate $\bm\xi_j$ \emph{dieses Knotens}
ausgewertet, auf Einheitslänge gebracht, über die angrenzenden Facetten summiert, und die Summe
wird erneut normiert (\code{compute\_normals}):
\begin{equation}
\bm n_j = \frac{\sum_{f\ni j}\hat{\bm n}_{j,f}}{\big\|\sum_{f\ni j}\hat{\bm n}_{j,f}\big\|},
\qquad
\hat{\bm n}_{j,f} = \frac{\bm n_{j,f}}{\|\bm n_{j,f}\|},
\qquad
\bm n_{j,f} =
\left.\frac{\partial\bm x}{\partial\xi}\times\frac{\partial\bm x}{\partial\eta}\right|_{\bm\xi_j}
\ \text{(3D)},
\qquad
\bm n_{j,f} = \bm R\left.\frac{\partial\bm x}{\partial\xi}\right|_{\xi_j}
\ \text{(2D)},
\label{eq:nodalnormal}
\end{equation}
mit $\bm R=\left[\begin{smallmatrix}0&1\\-1&0\end{smallmatrix}\right]$. Warum das wichtig ist,
sagt \citet[Abschn.~4.4]{farah} in einem Satz: die Knotennormalen sind
\emph{``of utmost importance \dots\ since they define the local direction in which the contact
force acts''}. Sie gehen ausschließlich \emph{normiert} ein -- als Richtung, nicht als Gewicht.

\paragraph{Zur Zuordnung der beiden Quellen.} \eqref{eq:nodalnormal} ist wörtlich
\citet[Gl.~(37)]{popp2010}, dort über die angrenzenden \emph{Slave}-Elemente gemittelt.
\citet[Gl.~(4.41)]{farah} hat dieselbe Form, ist dort aber für die \emph{Master}-Seite
formuliert; seine Slave-Normale folgt im Regelfall aus der Closest-Point-Projektion
(\citealp[Gl.~(4.42)]{farah}), und die Mittelung tritt bei ihm als Rückfall auf, wenn diese
Projektion versagt (\emph{``in rare cases the closest-point-projections fail the slave side nodal
normals are calculated via the well-established averaging procedure described in Popp et al.''}).
Gemeinsam ist beiden, dass sie \emph{Einheits}normalen summieren -- siehe den Absatz zur
Gewichtung.

\paragraph{Warum an der Knotenkoordinate und nicht je Facette.}
Die naheliegende Alternative ist \emph{eine} Normale je Facette, gebildet aus deren Eckknoten --
in 3D das Diagonalenkreuzprodukt $\bm n_f$ nach \eqref{eq:facetnormal}, in 2D die Eckknoten-Sehne.
Sie ist billiger, aber sie gibt allen Knoten einer Facette \emph{dieselbe} Richtung. Auf einer
quadratischen Facette bekommt der Kantenmittelknoten damit die Richtung seiner Ecken, und die
Krümmung der Facette erreicht die Normale überhaupt nicht -- die quadratischen Elementtypen
verhalten sich dann wie die linearen. Auf dem Zylinderausschnitt von Test~01 ist das mit einer
einzigen Zahlenreihe belegt (Abschnitt~\ref{sec:verif}):

\begin{center}
\small
\begin{tabular}{lrrl}
\toprule
Knotennormale & $n=2$ & $n=4$ & Fehlerreduktion \\
\midrule
je Facette (Eckknoten), CONQUAD8 & $7{,}1620^\circ$ & $3{,}5810^\circ$ & Faktor $2{,}00$ \\
je Knoten \eqref{eq:nodalnormal}, CONQUAD8 & $0{,}0278^\circ$ & $0{,}0035^\circ$ & Faktor $7{,}95$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Also nicht bloß genauer, sondern von anderer Ordnung: die facettenweise Variante fällt
linear und liefert für CONQUAD8 exakt denselben Winkelfehler wie das lineare CONQUAD4 --
der quadratische Ansatz war für die Normale wirkungslos. Auf einer \emph{ebenen} Slave-Fläche sind
beide Varianten dagegen identisch, und zwar exakt: dort sind alle Beiträge parallel, sodass jede
positive Gewichtung auf dieselbe Richtung normiert. Deshalb ändert sich in der gesamten
Referenzreihe (\file{Control\_Tests}, alle Interfaces eben) nichts über die Rundung hinaus, und
deshalb hat auch kein Löser-Testfall den Unterschied je gesehen.

\paragraph{Warum die Beiträge ungewichtet summiert werden.}
Die Gewichtung, mit der die angrenzenden Facetten in \eqref{eq:nodalnormal} eingehen, ist in der
Literatur nicht einheitlich; drei Varianten sind belegt:
\begin{itemize}\itemsep2pt
  \item \citet[Gl.~(37)]{popp2010} und \citet[Gl.~(4.41)]{farah} summieren die
        \emph{Einheits}normalen, also ungewichtet. Popp hält dazu ausdrücklich fest, das geschehe
        \emph{``for simplicity''}, die gewichtete Erweiterung nach \citet{yangmeng2005} sei
        \emph{``straightforward and has been tested as well, but does not change the results
        considerably''}.
  \item \citet[Gl.~(51)]{yangmeng2005} gewichten in 2D mit $\hat{\bm n}_A = l_2\bm n_{A1} +
        l_1\bm n_{A2}$, also \emph{invers} zur eigenen Kantenlänge -- dort zieht die
        \emph{kleinere} Facette stärker.
  \item Summiert man die \emph{nicht} normierten $\bm n_{j,f}$, so ist deren Länge die lokale
        Flächen-Jacobideterminante: dann zieht die \emph{größere} Facette stärker.
\end{itemize}
Der Code folgt der ersten Variante. Auf einem gleichmäßigen Netz sind alle drei identisch -- die
Beiträge sind dort gleich groß, jede positive Gewichtung normiert auf dieselbe Richtung --, und
weil die übrige Testreihe ausschließlich gleichmäßige Netze benutzt, sieht sie den Unterschied
nicht. Sichtbar wird er erst auf einem \emph{graduierten} Netz, und dort ist die ungewichtete Form
messbar genauer (Test~01, graduierter Zylinderausschnitt, benachbarte Facetten im Größenverhältnis
$2$, gemessen an den Knoten mit ungleich großen Nachbarn, Abschnitt~\ref{sec:verif}):

\begin{center}
\small
\begin{tabular}{lrr}
\toprule
Elementtyp & Einheitsnormalen \eqref{eq:nodalnormal} & flächengewichtet \\
\midrule
CONQUAD4, CONLINE2            & $1{,}9099^\circ$  & $3{,}8197^\circ$ \\
CONQUAD8, CONQUAD9, CONLINE3  & $0{,}0147^\circ$  & $0{,}0211^\circ$ \\
CONTRI3                       & $3{,}8254^\circ$  & $5{,}3509^\circ$ \\
CONTRI6                       & $0{,}0210^\circ$  & $0{,}0261^\circ$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Der Grund ist anschaulich: die größere Facette mittelt über einen größeren Bogen und ist
damit die \emph{schlechtere} Schätzung der Normalen an einem gemeinsamen Knoten. Ihr mehr Gewicht
zu geben zieht die Knotennormale von der exakten weg. Für den linearen Fall lässt sich das
geschlossen nachrechnen: bei Nachbargrößen $h$ und $2h$ liegt die ungewichtete Mittelung um $h/4$
neben der exakten Normale, die flächengewichtete um $h/2$ -- genau der gemessene Faktor $2$.

Ein zweiter Grund ist unabhängig von der Genauigkeit: die Flächen-Jacobideterminante trägt die
Größe des \emph{Referenz}elements mit. Ein Viereck der Fläche $A$ liefert $A/4$ (Referenzquadrat
$[-1,1]^2$), ein Dreieck derselben Fläche $2A$ (Einheitsdreieck) -- ein Faktor $8$. Eine
Flächengewichtung hinge damit am Elementtyp statt an der Geometrie, und zwei Netze derselben
Fläche, eines aus Vierecken und eines aus Dreiecken, bekämen an einer Knickstelle verschiedene
Knotennormalen. Mit Einheitsnormalen entfällt das ersatzlos.

Der \emph{entscheidende} Punkt -- die Auswertung an $\bm\xi_j$ statt je Facette -- ist von der
Gewichtung unberührt und in allen drei Quellen derselbe.

\paragraph{Knotenreihenfolge.}
\eqref{eq:nodalnormal} setzt voraus, dass die Tabelle der Knoten-Naturkoordinaten
(\code{getNodalNaturalCoordinates}) dieselbe Reihenfolge benutzt wie \code{getShapeFunctions}.
Ein Vertauscher darin gäbe einem Knoten stillschweigend die Normale eines anderen. Die Falle ist
real und liegt bei \code{CONLINE3} besonders nahe, dessen Reihenfolge Ende--Ende--\emph{Mitte}
ist, der letzte Listeneintrag also nicht der zweite Eckknoten. Geprüft wird deshalb die
definierende Eigenschaft, geometrieunabhängig und für alle sieben Elementtypen:
$N_a(\bm\xi_b)=\delta_{ab}$ (Abschnitt~\ref{sec:verif}).

\medskip
\noindent Die Mittelung über die angrenzenden Facetten garantiert ein stetiges Normalenfeld über
Facettengrenzen hinweg, was für die Konsistenz des Kontakts an Elementkanten nötig ist. Nicht
gemittelt wird dagegen die Normale der \emph{Segmentierung}: die Hilfsebene benutzt weiterhin die
facetteneigene $\bm n_c$ nach \eqref{eq:auxplane}, siehe Abschnitt~\ref{sec:hilfsebene}.
...
\begin{equation}
\frac{\max|\bm K - \bm K_{\text{FD,voll}}|}{\max|\bm K|} \;=\; 0{,}27 \ldots 1{,}17 ,
\end{equation}
...
\noindent Durchgehend gilt dabei $\bm K=-\partial\bm P_{\text{ext}}/\partial\bm U$. Erst diese
Konvention macht das Vorzeichen der aktiven Multiplikatorzeile lesbar: mit ihr folgt aus
$\text{PExt}[\lambda]\!+\!\!=\!\tilde g_j$ gerade $K[\lambda,\bm x_k]=+D_{jk}\bm n_j$, also
$K[\lambda,\bm x_k]=K[\bm x_k,\lambda]$ und damit die Symmetrie des Sattelpunktsystems. Das
umgekehrte Vorzeichen an dieser Stelle würde dieselbe Bedingung $\tilde g_j=0$ erzwingen, die
Multiplikatorzeile aber mit $-1$ skalieren und die Symmetrie zerstören. Test~09 prüft beides:
er vergleicht $\bm K$ mit dem Differenzenquotienten \eqref{eq:fdcheck} und weist zusätzlich aus,
dass nicht $\bm K\approx-\bm K_{\text{FD}}$ gilt -- ein Vorzeichendreher in der Assemblierung
fiele dort sofort auf.
...
Signorini-Bedingungen dann nicht. Das wird gemeldet (Abschnitt~\ref{sec:implnotes}) und ist in
den Löserläufen der Verifikationsreihe (\file{Control\_Tests}, Patch-Tests \file{07}/\file{08},
Tests \file{10}--\file{12}) nicht eingetreten. Ausgelöst wird die Meldung dort nur an einer
Stelle, und zwar absichtlich: der Regressionsfall in \file{06\_active\_set\_pdass} stellt genau
diese Konstellation her -- ein auf ``alles inaktiv'' eingefrorenes Increment, das nachträglich
eine Durchdringung erhält -- und prüft, dass der Constraint sie beim nächsten Increment auch
benennt.
...
Inkremente, gezählt über einen instrumentierten Lauf von \file{Control\_Tests/final\_report.py}
und den Patch-Tests \file{07}/\file{08}) friert das Set ausschließlich über das PDASS-Kriterium ein ($111$ Fälle), weder über
die Anti-Cycling-Regel noch über die Obergrenze; die höchste auftretende Iterationszahl beträgt
$11$ bei einer Schranke von $20$. Auch im Ausziehversuch (Abschnitt~\ref{sec:testbeispiel}) greift
die Obergrenze nicht: die zugehörige Laufzeitdiagnose \code{active\_set\_iteration\_cap} meldet sich
in keinem der beiden Läufe, obwohl einzelne Inkrementversuche in beiden die Iterationsgrenze des
Lösers selbst erreichen ($25$ Newton-Iterationen; konvergierte Inkremente brauchen höchstens $24$
in hex8 und $23$ in hex20). Das Set
ist dort also bereits vorher über das PDASS-Kriterium oder die Anti-Cycling-Regel eingefroren.
Greift die Obergrenze doch einmal, ist die Zulässigkeit der Lösung im Sinne der
...
% =====================================================================
% =====================================================================
\section{Penalty-Formulierung und augmentiertes Lagrange-Verfahren}
\label{sec:penalty}

Die bisherigen Abschnitte erzwingen die diskrete Zwangsbedingung $\tilde g_j=0$ über
Multiplikatoren, die als eigene Unbekannte im Gleichungssystem stehen. Dieselbe Bedingung lässt
sich auch ohne zusätzliche Unbekannte erzwingen, indem der Kontaktdruck unmittelbar aus der
Zwangsverletzung folgt. Beide Wege teilen die \emph{gesamte} geometrische Verarbeitung dieser
Arbeit -- Nachbarsuche, Projektion, Clipping, Segmentierung, Knotennormalen sowie $\bm D$ und
$\bm C$ -- und unterscheiden sich allein im Zwangsgesetz. Umgeschaltet wird mit dem Schlüsselwort
\code{formulation} (Anhang~\ref{sec:inputfile}).

Der Anlass ist nicht Bequemlichkeit. Eine \emph{explizite} Zeitintegration invertiert eine
Massenmatrix und braucht für jede Unbekannte eine Masse; ein Multiplikator hat keine. Ohne eine
multiplikatorfreie Formulierung ist der Kontakt einem expliziten Löser damit grundsätzlich
verschlossen.

% ---------------------------------------------------------------------
\subsection{Die Vorschrift}
\label{sec:penaltylaw}

\citet[Gl.~(24)]{puso2004} setzen den Knotendruck proportional zum gewichteten Spalt,
\begin{equation}
p_A \;=\; \kappa\,\bm g_A\cdot\bm\nu_A ,
\label{eq:penaltylaw}
\end{equation}
mit dem Penalty-Parameter $\kappa$; \citet[Gl.~(7)]{puso2008} schreiben dieselbe Beziehung als
$t_A=\kappa\,g_A$. Entscheidend ist, welche Größe dort $g_A$ heißt: nach
\citet[Gl.~(6)]{puso2004} ist es
$\bm g_A=\sum_{B,C}\int_\gamma[N^1_AN^1_B\bm x^1_B-N^1_AN^2_C\bm x^2_C]\,\mathrm d\gamma$, also
genau der \emph{gewichtete} Spalt dieser Arbeit \eqref{eq:wgap_pos} -- eine Größe der Dimension
Länge $\times$ Fläche, nicht die physikalische Knotenöffnung. Ebenso sind ihre Koppelgrößen
$n^i_{AB}=\int_\gamma N^1_AN^i_B\,\mathrm d\gamma$ \citep[Gl.~(13)]{puso2004} unsere $\bm D$ und
$\bm C$, und ihre Konsistenzbedingung $\sum_{B,C}(n^1_{AB}-n^2_{AC})=0$
\citep[Gl.~(15)]{puso2004} ist die Zeilensummen-Identität \eqref{eq:rowsum}.

Die Vorzeichenkonvention der Quelle ist die umgekehrte: dort steht die Nichtdurchdringung als
$g_A\le0$, hier ist $\tilde g_j/\tilde D_{jj}>0$ die offene Lage. Im Code trägt deshalb ein
eigenes Maß das Vorzeichen,
\begin{equation}
g_{\text{pen},j} \;=\; -\,\tilde g_j\,\operatorname{sgn}(\tilde D_{jj}),
\qquad
p_{n,j} \;=\; \max\bigl(0,\;z_j + \kappa\,g_{\text{pen},j}\bigr),
\qquad
\lambda_j \;=\; p_{n,j}\operatorname{sgn}(\tilde D_{jj}),
\label{eq:penaltynodal}
\end{equation}
positiv bei Durchdringung. Der Summand $z_j$ ist der Schätzer des augmentierten Verfahrens
(Abschnitt~\ref{sec:augmented}); für reines Penalty ist $z_j\equiv0$ und \eqref{eq:penaltynodal}
reduziert sich auf \eqref{eq:penaltylaw}. Dass $\lambda_j$ das Vorzeichen von $\tilde D_{jj}$
trägt, ist dieselbe Buchführung wie im Multiplikatorzweig (Abschnitt~\ref{sec:sgnd}) und sorgt
dafür, dass die Knotenkraft $-\lambda_j\tilde D_{jj}$ \eqref{eq:nodalforce} für beide Vorzeichen
des Gewichts drückt.

\paragraph{Warum der gewichtete Spalt und nicht die Öffnung.}
Die kontinuierliche Penalty-Regularisierung setzt am \emph{punktweisen} Spalt an:
\citet[Gl.~(30)/(31)]{wriggers} schreibt
$\Pi^P_c=\int(\varepsilon_N g_N\,\delta g_N+\varepsilon_T\bm g_T\!\cdot\!\delta\bm g_T)\,\mathrm dA$
mit $\varepsilon_N>0$, hält fest, dass die Lagrange-Lösung für $\varepsilon_N\to\infty$
wiedergewonnen wird, und warnt zugleich vor der schlechten Kondition bei großem
$\varepsilon_N$. Diskretisiert man diese Form konsistent, erhielte man
$p_j=\varepsilon_N\,\tilde g_j/\tilde D_{jj}$, also den Penalty-Parameter auf der physikalischen
Öffnung. Beide Lesarten hängen über $\kappa=\varepsilon_N/\tilde D_{jj}$ zusammen und streben für
$\kappa\to\infty$ gegen dieselbe Grenze, weil beide $\tilde g_j\to0$ erzwingen.

Umgesetzt ist die Form der beiden Puso-Arbeiten, und zwar aus einem Grund, der in dieser
Implementierung besonders schwer wiegt: \eqref{eq:penaltynodal} \textbf{dividiert nicht durch
$\tilde D_{jj}$}. Das Knotengewicht ist hier nicht sicher von null weg -- ein teilweise
überdecktes \code{CONQUAD9}, eine \code{CONQUAD8}-Ecke bei $\alpha=\tfrac13$ und der
Sliver-Rückfall erzeugen kleine und sogar negative Gewichte (Abschnitte~\ref{sec:alphachoice}
und~\ref{sec:sliver}). In der Form $p_j=\varepsilon_N\,\tilde g_j/\tilde D_{jj}$ stünde genau
dieses Gewicht im \emph{Nenner des Kontaktdrucks}; der Multiplikatorzweig muss die Division
deshalb eigens abfangen (Schwelle \code{current\_D\_tol}), und ein abgefangener Knoten wäre in der
Penalty-Form ein Knoten ohne Kontaktkraft.

\paragraph{Abgrenzung.} Nicht gemeint ist die in Abschnitt~\ref{sec:ausblick:kondensation}
beschriebene Abkürzung $\lambda_j=-\tilde g_j/\tilde D_{jj}$. Deren Ersatzsteifigkeit
$1/\tilde D_{jj}$ stammt aus der Facettengeometrie und hat mit der Steifigkeit der Körper nichts
zu tun. In \eqref{eq:penaltylaw} ist $\kappa$ dagegen ein frei wählbarer Parameter, der genau
diese Rolle übernimmt.

% ---------------------------------------------------------------------
\subsection{Linearisierung}
\label{sec:penaltytangent}

Mit den Gewichten $w_a=+D_{jk}$ auf den Slave-Knoten und $w_a=-C_{jl}$ auf den Master-Knoten ist
der gewichtete Spalt $\tilde g_j=-\sum_a w_a(\bm x_a\!\cdot\!\bm n_j)$, also
$\partial\tilde g_j/\partial\bm x_a=-w_a\bm n_j$. Mit $\bm v_a=w_a\bm n_j$ folgt aus
\eqref{eq:penaltynodal} unmittelbar
\begin{equation}
\bm P_{\text{ext},a} \;\mathrel{-}=\; \lambda_j\,\bm v_a ,
\qquad
\bm K_{ab} \;\mathrel{+}=\; \kappa\,\bm v_a\otimes\bm v_b .
\label{eq:penaltytangent}
\end{equation}
Der Knotenbeitrag ist damit vom Rang eins und symmetrisch positiv semidefinit -- das System bleibt
positiv definit, es entsteht kein Sattelpunkt, und die Zahl der Unbekannten sinkt um einen
Multiplikator je Slave-Knoten. Das $\operatorname{sgn}(\tilde D_{jj})$ aus
\eqref{eq:penaltynodal} hebt sich in der Ableitung heraus.

\eqref{eq:penaltytangent} ist der \emph{erste} Term der vollständigen Linearisierung
\citep[Gl.~(25)]{puso2004}
$\Delta\bm f^i_{AB}=\kappa n^i_{AB}(\bm\nu_A\otimes\bm\nu_A)\Delta\bm g_A
+n^i_{AB}(p_A\bm I+\bm\nu_A\otimes\bm g_A)\Delta\bm\nu_A+p_A\bm\nu_A\Delta n^i_{AB}$.
Die beiden übrigen Terme -- die Linearisierungen der Knotennormale und der Koppelgewichte --
entfallen hier, weil die Geometrie im Increment eingefroren ist
(Abschnitt~\ref{sec:frozen}). Die assemblierte Tangente ist damit die \emph{exakte} Jacobi-Matrix
des eingefrorenen Residuums, nicht die des vollen Problems; das ist dieselbe Einschränkung, unter
der auch der Multiplikatorzweig steht, und derselbe offene Punkt im Ausblick.

% ---------------------------------------------------------------------
\subsection{Aktivmenge}
\label{sec:penaltyactiveset}

Der Penalty-Zweig braucht weder eine Komplementaritätsfunktion noch den Parameter $c_n$: die
Regularisierung \emph{ist} das Gesetz. Aktiv ist ein Knoten, wenn $z_j+\kappa g_{\text{pen},j}>0$
ist -- bei reinem Penalty also genau dann, wenn er durchdringt. Das ist die Freigabebedingung des
Algorithmus von \citet[Schritt~(3)]{puso2008}.

Bemerkenswert ist die strukturelle Verwandtschaft: der semismoothe Indikator
$s_n=p_n-c_n\,g_{\text{sep}}$ des Multiplikatorzweigs \eqref{eq:ncp} und der augmentierte Druck
$z_j+\kappa g_{\text{pen},j}$ sind dasselbe Objekt, von zwei Seiten betrachtet -- ein
Multiplikatorschätzer plus ein Vielfaches der Zwangsverletzung. Genau diese Beobachtung liegt der
Deutung der primal-dualen Aktivmengenstrategie als semismoothes Newton-Verfahren zugrunde
\citep{hik2002,hueber2005}. Eine Konsequenz ist praktischer Natur: weil die Menge dem Druck folgt
und nicht iteriert wird, entfallen im Penalty-Zweig das Einfrieren der Aktivmenge und die
Anti-Cycling-Buchführung aus Abschnitt~\ref{sec:loesung}.

% ---------------------------------------------------------------------
\subsection{Augmentiertes Lagrange-Verfahren}
\label{sec:augmented}

Reines Penalty erfüllt die Zwangsbedingung nur bis auf $O(1/\kappa)$, und $\kappa$ steht damit im
Ergebnis. Beide Puso-Arbeiten umgehen das mit einer äußeren Schleife: \citet[Gl.~(18)]{puso2008}
ziehen den Druckschätzer nach
\begin{equation}
z^{(k+1)}_j \;=\; \max\bigl(0,\;z^{(k)}_j+\kappa\,g_{\text{pen},j}\bigr),
\label{eq:uzawa}
\end{equation}
und zwar ausdrücklich erst, \emph{``once convergence of the Newton--Raphson loop is achieved''};
danach wird das Gleichgewicht mit dem neuen Schätzer neu hergestellt. Im Grenzfall
$g_{\text{pen},j}\to0$ ist $\lambda_j\to z_j$, und wo $z_j$ landet, entscheidet das Gleichgewicht
-- nicht $\kappa$. Der Parameter steuert dann nur noch die Rate der äußeren Schleife.

\paragraph{Abbruch.} \citet{puso2008} lassen die Toleranz offen und nennen zwei Möglichkeiten,
entweder die Nichtdurchdringung direkt zu prüfen oder ``changes of multipliers from iteration to
iteration'' zu verfolgen. Umgesetzt ist die zweite, als \emph{relative} Änderung
$\max_j|z^{(k+1)}_j-z^{(k)}_j|\le\texttt{tol}\cdot\max_j|z^{(k+1)}_j|$. Der Grund ist derselbe wie
in Abschnitt~\ref{sec:scalelimit}: eine Schranke am gewichteten Spalt trüge die Einheit
Länge $\times$ Fläche und wäre nicht skaleninvariant, die Multiplikatorform ist dimensionslos.

\paragraph{Einbettung.} Die Schleife ist echt geschachtelt und setzt eine Gleichgewichtsiteration
voraus, in die sie sich einhängen kann. Für einen expliziten Löser steht sie damit nicht zur
Verfügung -- dort bleibt nur reines Penalty. Zu Beginn jedes Increments wird $z_j$ aus dem letzten
konvergierten Wert warm gestartet; ein Rückschritt nach Cutback setzt ihn aus demselben Grund
zurück wie die eingefrorene Geometrie (Abschnitt~\ref{sec:frozen}). Dass die Kontaktkräfte damit
zu Increment-Beginn nicht diejenigen sind, die den eingehenden Verschiebungszustand erzeugt haben,
hat eine Folge für den Löser: der Zustand darf nicht ungeprüft als konvergiert angenommen werden,
bevor eine Korrektur gerechnet wurde.

% ---------------------------------------------------------------------
\subsection{Wahl von $\kappa$}
\label{sec:kappachoice}

Für $\kappa$ gibt \textbf{keine} der beiden Quellen eine Regel an; \citet{puso2008} sprechen
lediglich von ``a suitable penalty parameter''. Der Vorgabewert dieser Implementierung ist deshalb
als eigene Ingenieurregel zu lesen und wird beim ersten Assemblieren einmal gemeldet:
\begin{equation}
\kappa \;=\; C\,\frac{E}{h\,\bar D},
\qquad C=100,
\qquad \bar D=\frac{|\gamma_c^{\text{ovl}}|}{n_{\text{slave}}},
\qquad h=\Bigl(\frac{|\gamma_c^{\text{ovl}}|}{n_{\text{facets}}}\Bigr)^{1/(d-1)} .
\label{eq:kappadefault}
\end{equation}
Dabei ist $E$ der kleinste Anfangs-Elastizitätsmodul der angrenzenden Materialien -- dieselbe
Begründung wie bei $c_n$ (Abschnitt~\ref{sec:ncp}) --, und $\bar D$ sowie $h$ folgen aus den
Mortar-Gewichten selbst, deren Zeilensumme nach \eqref{eq:rowsum} das überdeckte Interfacemaß
ergibt. Der Sinn ist, dass $\varepsilon_N=\kappa\bar D=C\,E/h$ die Steifigkeit einer angrenzenden
Elementlage trifft.

Der Faktor $C$ kauft die Konvergenzrate der äußeren Schleife: sie fällt näherungsweise wie
$k_{\text{Struktur}}/(k_{\text{Struktur}}+k_{\text{Kontakt}})$. Gemessen an der Zwei-Block-Reihe
(\file{Control\_Tests} 03, hex8, $E=1000$) reduziert $C=1$ die Zwangsverletzung je Runde um den
Faktor $0{,}51$ und braucht rund $25$ Augmentierungen für eine relative $10^{-6}$, während $C=100$
mit \emph{vier} auskommt -- Restdurchdringung $4{,}4\cdot10^{-5}\to3{,}5\cdot10^{-7}\to
3{,}4\cdot10^{-9}\to3{,}5\cdot10^{-11}$. Mit eingeschalteter Augmentierung hängt das konvergierte
Ergebnis nicht von $\kappa$ ab; die Regel muss daher vernünftig sein, nicht richtig. Ohne
Augmentierung steht $\kappa$ im Ergebnis und ist bewusst zu wählen.

% ---------------------------------------------------------------------
\subsection{Grenzen der Penalty-Formulierung}
\label{sec:penaltylimits}

\paragraph{Erzwungen wird der gewichtete Spalt.} \eqref{eq:penaltynodal} begrenzt $\tilde g_j$,
nicht die physikalische Öffnung $\tilde g_j/\tilde D_{jj}$. Wo das Knotengewicht klein wird --
also am Rand der Kontaktzone und bei Splitterüberdeckung -- verstärkt die Division die
verbleibende Verletzung. Gemessen in \file{10\_signorini\_check} an Knoten mit
$\tilde D_{jj}/\max_k|\tilde D_{kk}|=2{,}5\cdot10^{-5}\ldots1{,}4\cdot10^{-4}$: die Öffnung
erreicht dort $1{,}5\cdot10^{-3}$, wo der Sattelpunkt $10^{-16}$ hält. Die \emph{Knotenkraft}
bleibt mit $1{,}4\cdot10^{-5}$ ohne Belang für eine Lastaussage, die Signorini-Bedingungen werden
aber punktweise geprüft und sind dort verletzt. Das ist die Kehrseite genau des Vorteils aus
Abschnitt~\ref{sec:penaltylaw} und keine Ungenauigkeit der Umsetzung.

\paragraph{Teilweise überdeckte Randknoten.} Dort verteilen die beiden Formulierungen den Druck
messbar verschieden. In den Gleit-Testfällen der Zwei-Block-Reihe
(\file{Control\_Tests} 05, alle sechs Varianten) gleitet der Master über die Slave-Kante hinaus;
der Multiplikatorzweig hält $\lambda_j$ gleichförmig, der Penalty-Zweig hebt den Druck an den
angeschnittenen Randknoten an. Die Folge ist ein Schub von $2{,}3\cdot10^{-2}\ldots7{,}0\cdot
10^{-2}$ gegen $5\cdot10^{-15}\ldots10^{-11}$ im Multiplikatorzweig, obwohl der Kontakt
reibungsfrei ist. Der Unterschied verschwindet \emph{nicht} mit der Schrittweite, ist also kein
Staffelungsfehler, und die Assemblierung ist für gleiches $\lambda_j$ nachweislich identisch.
Welche der beiden Verteilungen die richtige ist, ist ungeklärt; die Frage hängt mit der Deutung
des Multiplikators bei Teilüberdeckung zusammen (Abschnitt~\ref{sec:lambdadeutung}) und ist hier
...
Division wird bei $|\tilde D_{jj}|\le10^{-12}\max_k|\tilde D_{kk}|$ durch $0$ ersetzt und
$\mathrm{sgn}(0)=0$ gesetzt, sodass
\begin{equation}
...
$70\,\%$ Überdeckung, \file{06\_active\_set\_pdass}) bis $-0{,}60$ (Sliver-Rückfall,
\file{hertz\_hex20\_medium}) relativ zum größten Gewicht.
...
quadratische Flächen CONQUAD8 verwenden, CONQUAD9 nur bei voller Überdeckung -- mit der
Einschränkung, dass auch CONQUAD8 keine Positivitätsgarantie bei Teilüberdeckung besitzt. Sein
Negativgebiet ist nur klein (Abschnitt~\ref{sec:alphachoice}); auf dem Rückfallpfad entfällt auch
dieser Vorteil (Abschnitt~\ref{sec:sliver}).

\paragraph{Tragweite für das Ergebnis.}
Wie weit die Sache trägt, lässt sich für CONQUAD8 auf dem konsistenten Weg genau eingrenzen -- drei
Aussagen, die zusammen erklären, warum ein negatives Knotengewicht ein Genauigkeitsbefund an
wenigen Randknoten bleibt und keine Aussage über die übertragene Last gefährdet.

\emph{Erstens} bleibt die Kraftübertragung unberührt. Die Zeilensummen-Identität
$\sum_K D_{IK}=\sum_J C_{IJ}$ \eqref{eq:rowsum} gilt unabhängig vom Vorzeichen des Gewichts und
wird auch an Knoten mit $\tilde D_{jj}<0$ auf $10^{-16}$ eingehalten
(\file{05\_quadratic\_segmentation}, in denselben Konfigurationen mitgeprüft, in denen das Gewicht
kippt). Was die Slave-Seite abgibt, kommt auf der Master-Seite an; eine Reaktionskraft-Auswertung
wie im Ausziehversuch kann durch negative Gewichte weder gewinnen noch verlieren.

\emph{Zweitens} sind die tragenden Knoten strukturell ausgeschlossen. Für ein \emph{voll}
überdecktes Slave-Element ist $\int_{\gamma_c^{\text{ovl}}}\tilde N_j=\int_{\text{ele}}\tilde N_j>0$,
und das gilt bereits ab $\alpha>\tfrac18$. Betroffen sein können also nur Knoten am \emph{Rand der
Kontaktzone} -- dort, wo das Slave-Netz über das Master-Netz hinausragt und der überdeckte Anteil
im Negativgebiet der Eckfunktion liegt.

\emph{Drittens} ist ein solches Gewicht der Größe nach beschränkt. Aus der punktweisen Schranke
$\tilde N_j\ge-\tfrac{1}{324}$ (Abschnitt~\ref{sec:alphachoice}) folgt unmittelbar
\begin{equation}
\tilde D_{jj} \;\ge\; -\tfrac{1}{324}\,\big|\gamma_c^{\text{ovl}}\big| ,
\label{eq:negweightbound}
\end{equation}
also höchstens $0{,}31\,\%$ der überdeckten Fläche -- gegenüber $\tfrac{5}{36}=13{,}9\,\%$, die
derselbe Eckknoten bei voller Überdeckung trägt. Das ist ein Faktor $45$. Und da die Knotenkraft
$-\lambda_j\tilde D_{jj}$ ist \eqref{eq:nodalforce}, ist der Beitrag eines solchen Knotens zur
übertragenen Kraft um denselben Faktor kleiner als der eines voll überdeckten. Die drei
Messpunkte aus Abschnitt~\ref{sec:alphachoice} halten \eqref{eq:negweightbound} mit Faktor $2$ bis
$4{,}5$ Abstand ein.

Offen bleibt genau eine Sache, und die ist nicht durch obige Schranke gedeckt: die \emph{lokale}
Durchdringung an einem solchen Knoten wird nicht automatisch bemerkt, weil $\tilde g_j$ dort seine
Deutung verloren hat. Wer sie ausschließen will, prüft den physikalischen Spalt am konvergierten
Zustand (\file{10\_signorini\_check}) statt den gewichteten. Und \eqref{eq:negweightbound} gilt
\emph{nur} auf dem konsistenten Weg: auf dem Rückfallpfad erreichen die Gewichte $-1{,}0$ des
größten Gewichts (Abschnitt~\ref{sec:sliver}), dort greift keine der drei Eingrenzungen außer der
ersten.
...
jede Grenze. Gemessen am Rand der Teilüberdeckung (\file{10\_signorini\_check}, Lastfall
\code{teilkontakt}): $\tilde D_{jj}/\max_k|\tilde D_{kk}|=1{,}5\cdot10^{-4}$ (CONLINE3) und
$2{,}7\cdot10^{-5}$ (CONLINE2) ergeben $\lambda_j=1{,}2\cdot10^{4}$ und $1{,}0\cdot10^{5}$,
während die Knotenkraft mit $0{,}35$ bzw.\ $0{,}68$ von der Ordnung eins bleibt. Der Test weist
solche Knoten eigens aus (Meldung ``Splitterüberdeckung'', Schwelle
$|\tilde D_{jj}|<10^{-3}\max_k|\tilde D_{kk}|$).
...
Referenzkoeffizienten stammen aus \code{element.computeLocalMassMatrices} und werden
\emph{nur in diesem Zweig} gebildet, für die betroffene Facette allein -- sie kosten eine
Elementmassenmatrix samt Inversion, und der Zweig wird nach der Zählung im folgenden Absatz in
$4$ von $3\,989$ Slave-Element-Auswertungen betreten. (Dieselbe Größe für \emph{alle}
Slave-Facetten liefert \code{compute\_local\_dual\_matrices}; sie wird von
\file{04\_dual\_biorthogonality} benutzt, aber nicht mehr vom Produktionspfad.) Für diese Zellen
gilt die Biorthogonalität dann nur auf dem Voll-Element, nicht auf der Überlappung.

\paragraph{Häufigkeit des Rückfallpfads.}
Über die \file{Control\_Tests}-Reihe und die Patch-Tests -- $251$ Inkremente mit $3\,989$
ausgewerteten Slave-Elementen, gezählt über einen instrumentierten Lauf von
\file{Control\_Tests/final\_report.py} und den Patch-Tests \file{07}/\file{08};
\file{POT\_Dejori} ist nicht Teil dieser Auswertung -- wird der Pfad
...
Auf dem konsistenten Weg ist $\tilde D_{jj}=\int_{\text{Überlappung}}\tilde N_j$. Für CONQUAD8 ist
diese Größe bei Teilüberdeckung durch die Basistransformation \emph{nicht} abgesichert:
$\alpha=\tfrac13$ sichert nur das Voll-Element-Integral; punktweise nichtnegativ wäre $\tilde N_j$
erst ab $\alpha\ge\tfrac38$ \eqref{eq:alphastar}. Liegt die Überlappung überwiegend im
verbleibenden Negativgebiet der Eckfunktion, wird das Gewicht auch ohne Rückfall negativ --
gemessen ab $14\,\%$ Eckenüberdeckung, bei $9\,\%$ mit $-0{,}26\,\%$ des größten Gewichts
(Abschnitt~\ref{sec:alphachoice}).

Der Rückfallpfad verschärft das um Größenordnungen. Dort steht an derselben Stelle
$\int_{\text{Überlappung}}\Phi_j^{\text{ref}}$ mit den dualen Funktionen des \emph{Voll}-Elements.
Diese sind punktweise vorzeichenwechselnd -- ihre definierende Eigenschaft ist die
Biorthogonalität \eqref{eq:biorth} über das \emph{ganze} Element, nicht Nichtnegativität --, und das
Integral über ein Teilgebiet kann daher jedes Vorzeichen und jede Größenordnung annehmen. Ein
...
beiden hex20-Hertz-Modellen treten dadurch in $5$ ihrer zusammen $22$ Inkremente Knoten mit
$\tilde D_{jj}<0$ auf, je Increment zwei bis vier, im ungünstigsten Fall bei $-60\,\%$ des
größten Gewichts (\file{hertz\_hex20\_medium}: $3$ Inkremente, schlechtestes Verhältnis $-0{,}60$;
\file{hertz\_hex20\_fine}: $2$ Inkremente, $-0{,}015$). Die vier CONQUAD4-Varianten der Reihe
bleiben frei davon. Diese Zahlen stammen aus einem instrumentierten Lauf: die Laufzeitdiagnose
selbst meldet je Constraint nur das \emph{erste} Auftreten und beziffert daher weder die Anzahl
der betroffenen Increments noch das schlechteste Verhältnis. Die
Vorzeichenbehandlung aus Abschnitt~\ref{sec:sgnd} deckt diesen Fall ab, sodass die Lösungen
gültig bleiben; negative Knotengewichte sind damit aber keine Eigenheit des CONQUAD9. Sie treten
bei jeder Sliver-Zelle unabhängig vom Elementtyp auf -- und beim CONQUAD8 zusätzlich, wie oben
gezeigt, schon ohne Sliver, sobald die Überdeckung im Negativgebiet der Eckfunktion liegt. Die
Konditionsschranke $10^{12}$
...
über die drei Skalen bis $10^{-12}$ überein. Für $k=10^{-3}$ übersteigt der Suchrand die
...
\begin{center}
\small
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{5.6cm}>{\raggedright\arraybackslash}p{2.4cm}lX}
\toprule
Toleranz & Einheit & Pfad & geprüft durch \\
\midrule
Suchmarge $\max(0{,}15\,L,\,0{,}1)$ & Länge & 2D+3D & Test~11 (beide Varianten) \\
\code{tol} $=10^{-12}$ in \code{map\_2d\_to\_natural} & Länge (Residuum), sonst dimensionslos & 2D+3D & Test~11 (beide Varianten) \\
...
$10^{-14}$ in \code{is\_convex\_polygon} & \textbf{Fläche} & \textbf{nur 3D} & 3D-Variante von Test~11 \\
$\|\bm n_f\|<10^{-14}$ (entartete Sub-Zelle) & \textbf{Fläche} & \textbf{nur 3D} & 3D-Variante von Test~11 \\
$L_{\text{slave}}<10^{-14}$ und Überlappung $<10^{-12}$ in \code{clip\_1d\_segments} & Länge & \textbf{nur 2D} & 2D-Variante von Test~11 \\
$\|\sum_f \bm n_{j,f}\|<10^{-14}$ in \code{compute\_normals} & Fläche (3D), Länge (2D) & 2D+3D & Test~11 (beide Varianten) \\
$|\tilde D_{jj}|\le10^{-12}\max_k|\tilde D_{kk}|$ (Division im Indikator) & \textbf{relativ} & 2D+3D & Test~11 (beide Varianten) \\
$|D_{jk}|>10^{-12}\max_{ab}|D_{ab}|$ (Besetzungsmuster) & \textbf{relativ} & 2D+3D & Test~11 (beide Varianten) \\
\bottomrule
\end{tabularx}
\end{center}

\noindent Die beiden letzten Zeilen sind die einzigen, die \emph{keine} Modelleinheit tragen, und
das ist Absicht. Beide betreffen Einträge von $\bm D$, also Integrale $\int\Phi_j\tilde N_k$ mit
der Einheit einer Fläche; eine absolute Schranke wäre dort eine Aussage über die Längeneinheit des
Modells und nicht über die Arithmetik. Zu trennen ist Rundungsstaub -- der bauartbedingt bei
$\varepsilon\max|\bm D|$ liegt, unabhängig von der Einheit -- von einem echten Kopplungsterm, und
$10^{-12}\max|\bm D|$ liegt vier Größenordnungen über dem Staub und zwölf unter dem Signal, für
jede Längenskala. Bei der Division im Indikator kommt ein zweites Argument hinzu: eine sehr kleine
absolute Schranke \emph{teilt noch}. Bei $\tilde D_{jj}=10^{-25}$ und einer Schranke von $10^{-30}$
-- dem Wert, der hier zuvor stand -- explodierte $g_{\text{sep},j}=\tilde g_j/\tilde D_{jj}$ um
fünfundzwanzig Größenordnungen, und das Vorzeichen dieses Rundungsprodukts entschiede über den
Zweig der NCP: ein Knoten ließe sich so allein durch Rundung \emph{aktiv} schalten, mit einer
Zwangsbedingung ohne Bedeutung. Relativ zum größten Knotengewicht der Grenzfläche kann das nicht
passieren; das kleinste über die Testreihe gemessene Gewicht eines überdeckten Knotens liegt bei
$\sim\!10^{-3}$ des größten. Der Wechsel ist über die gesamte Referenzreihe (\file{Control\_Tests},
$46$ Läufe) und die Tests~01--12 \emph{bitgleich} zum vorherigen Stand.
...
Verschwindende Knotennormale & Eindeutigkeit der Normalen \eqref{eq:nodalnormal}. Der Knoten
...
\paragraph{Was \emph{nicht} gemeldet wird.} Zwei Rückfälle in der Segmentierung laufen stumm, und
das gehört zur Aufzählung, weil sonst der Eindruck entstünde, jede Entartung sei sichtbar:
\begin{itemize}\itemsep2pt
  \item Eine \textbf{entartete Sub-Zelle} wird übersprungen -- in 3D bei verschwindender
        Facettennormale ($\|\bm n_f\|<10^{-14}$, \code{compute\_mortar\_coupling\_matrices}), in
        2D bei verschwindender Sub-Zellen-Länge ($<10^{-14}$, \code{clip\_1d\_segments}). Ihr
        Beitrag zu $\bm D$ und $\bm M$ entfällt damit ohne Hinweis. Ebenso werden
        Integrationsdreiecke mit $J_c<10^{-14}$ und 1D-Überlappungen unter $10^{-12}$ verworfen.
        Das ist für echte Splitterzellen gewollt (ihr Beitrag ist ohnehin null), setzt aber
        voraus, dass die verworfene Fläche tatsächlich verschwindet -- eine Annahme über die
        Netzqualität, keine geprüfte Eigenschaft.
  \item Im Clipping fängt \code{sutherland\_hodgman\_clip} den Fall \textbf{exakt paralleler
        Kanten} (Determinante genau null bei unterschiedlich klassifizierten Endpunkten) durch
        Rückgriff auf den kreuzenden Eckpunkt ab. Das hält das Polygon geschlossen und
        degeneriert die entstehende Splitterzelle auf null Fläche, die der Aufrufer verwirft;
        ohne diesen Zweig entstünden \code{inf}/\code{NaN} und verdürben die ganze Zelle.
\end{itemize}

\medskip
\noindent Hinzu kommen zwei \emph{informative} Meldungen, die keine Voraussetzung verletzen, sondern eine
...
Über die Testreihe hinweg melden sich vier der zehn Zustände, und zwar jeder dort, wo er soll:

\begin{itemize}\itemsep2pt
  \item \textbf{negatives Knotengewicht} und \textbf{Selbstprüfung des Active Sets} in den beiden
        Regressionsfällen von \file{06\_active\_set\_pdass}, die eben diese Konfigurationen
        herstellen (teilüberdecktes CONQUAD9 bzw.\ ein eingefrorenes Set, das nachträglich eine
        Durchdringung erhält);
  \item \textbf{nicht-konvexe Slave-Sub-Zelle} und \textbf{Sliver-Rückfall} in
        \file{05\_quadratic\_segmentation}, das die zugehörigen Schwellen vermisst und die
        Meldungen dabei als Teil der Prüfung abfängt;
  \item \textbf{Sliver-Rückfall und negatives Knotengewicht gemeinsam} -- im Lastfall
        \code{teilkontakt} von \file{10\_signorini\_check} (CONLINE2/3 am Rand der Überdeckung,
        $\mathrm{cond}(\bm M_t)=3{,}7\cdot10^{13}$, ein Knoten bei $-7{,}1\cdot10^{-3}$ des
        größten Gewichts) und auf den beiden hex20-Hertz-Modellen der \file{Control\_Tests}. Das
        bestätigt den in Abschnitt~\ref{sec:sliver} beschriebenen Zusammenhang zweimal
        unabhängig, in 2D und in 3D.
\end{itemize}

\noindent Die übrigen sechs Zustände treten über die gesamte Reihe nicht auf: Obergrenze der
Active-Set-Iteration, fehlgeschlagene Rückabbildung, verschwindende Knotennormale, nicht-konvexe
Master-Sub-Zelle und die beiden Orientierungsprüfungen. Die Modelle mit \emph{CONQUAD4}
(Patch-Tests, Zwei-Block-Reihe, hex8-Hertz) bleiben von negativen Knotengewichten frei: dort ist
die Basistransformation die Identität und die bilinearen Formfunktionen sind punktweise
nichtnegativ, sodass $\tilde D_{jj}>0$ auch bei Teilüberdeckung gilt -- solange der Rückfallpfad
nicht greift, der diese Zusage für jeden Elementtyp aufhebt.

Auf dem Ausziehversuch (Abschnitt~\ref{sec:testbeispiel}, CONQUAD4) spricht als einzige Diagnose
die Selbstprüfung des Active Sets an, und zwar auf zwei der sechs Kontaktpaare -- den beiden
Schaftflanken -- an je zwei Slave-Knoten. Da die Meldung je Constraint und Ursache nur
\emph{einmal} ausgegeben wird, steht damit fest, \emph{dass} sie an diesen beiden Kontaktpaaren
mindestens einmal ausgelöst hat -- nicht, wie oft
und in welchen Increments. Und da dieselbe Rechnung $17$ Cutbacks enthält, ist nach der oben
...
Die quadratische Variante desselben Modells (CONQUAD8) meldet dieselbe Selbstprüfung ebenfalls auf
allen sechs Kontaktpaaren und darüber hinaus \emph{negative Knotengewichte}: auf dem Kontaktpaar
\code{contact\_l\_head\_vert} an acht Slave-Knoten, im ungünstigsten Fall bei $-1{,}000$ des größten
Gewichts -- das negative Gewicht ist dort also dem Betrag nach das größte dieser Fläche --, und das
bereits ab Increment~1. Der Sliver-Rückfall spricht dabei \emph{nicht} an: es ist
der in Abschnitt~\ref{sec:alphachoice} beschriebene Fall, in dem die Überdeckung eines
Slave-Elements im Negativgebiet der transformierten Eckfunktion liegt und $\tilde D_{jj}$ schon auf
dem konsistenten Weg negativ wird. Das gemeldete Verhältnis ist relativ zum größten Gewicht
\emph{derselben} Fläche; über die absolute Größe der Gewichte sagt es nichts, und die Diagnose
nennt nur das erste Auftreten. Die Vorzeichenbehandlung aus Abschnitt~\ref{sec:sgnd} trägt den
Fall; der gewichtete Spalt ist an diesen Knoten aber kein mittleres Öffnungsmaß mehr.

Für die Bewertung der Kurven in Abschnitt~\ref{sec:testbeispiel} ist das ohne Belang, und zwar aus
den in Abschnitt~\ref{sec:sgnd} genannten Gründen: die Zeilensummen-Identität und damit die
übertragene Kraft sind vom Vorzeichen des Gewichts unabhängig, betroffen sein können nur Knoten am
Rand der Kontaktzone -- die tragenden, voll überdeckten sind ausgeschlossen --, und die Größe eines
negativen Gewichts ist nach \eqref{eq:negweightbound} auf $\tfrac{1}{45}$ dessen beschränkt, was
derselbe Knoten bei voller Überdeckung trüge. Der Umfang passt dazu: $8$ der $43$ Slave-Knoten
\emph{eines} der sechs Kontaktpaare, bei $498$ Multiplikatorzeilen im Modell, also rund
$1{,}6\,\%$.
Was offenbleibt, ist ausschließlich eine mögliche lokale Durchdringung an diesen Knoten; sie wäre
über den physikalischen Spalt am Endzustand zu prüfen, nicht über $\tilde g_j$.
...
Eigenschaft & Prüfung \\
\midrule
\textbf{Knotennormalen} & Gemittelte Knotennormale \eqref{eq:nodalnormal} gegen die \emph{analytische} Normale eines Zylinderausschnitts, mit einer geforderten Mindest-Fehlerreduktion unter Netzverfeinerung; auf einem \emph{graduierten} Netz zusätzlich die Gewichtung zwischen den Nachbarfacetten, die auf gleichmäßigen Netzen prinzipiell unsichtbar ist; dazu die Orientierung (nach außen) und die Übereinstimmung der Knoten-Naturkoordinaten mit den Formfunktionen, $N_a(\bm\xi_b)=\delta_{ab}$, für alle sieben Elementtypen (\file{01\_node\_normals}). \\
...
\textbf{Quadratische Segmentierung} & Sub-Cell-Zerlegung + Segmentquadratur für CONQUAD8/9, CONTRI6; Schwellen der Sub-Cell-Konvexität und Übergang zum Rückfallpfad samt Vorzeichenwechsel von $\tilde D_{jj}$. Dazu die Grenze der Basistransformation: bei $\alpha=\tfrac13$ ist die CONQUAD8-Eckfunktion nicht punktweise nichtnegativ, und das knotenweise Gewicht kippt schon auf dem \emph{konsistenten} Pfad, sobald die Überlappung im Negativgebiet liegt -- gegen die geschlossene Form $-\tfrac{1}{36}(s-1)^4(8s-5)$ gerechnet, Vorzeichenwechsel bei $s=\tfrac58$ (\file{05\_quadratic\_segmentation}). \\
...
\textbf{Patch-Test hex20} & 3D-Patch-Test, vier Netz-/Ordnungskombinationen mit geraden Kanten plus eine verzerrte Variante (\file{08\_patch\_test\_hex20}). Gepaart werden CONQUAD8/CONQUAD8 (passend und nichtpassend) sowie beide gemischten Ordnungen, CONQUAD8-Slave gegen CONQUAD4-Master und umgekehrt. \\
...
\begin{itemize}\itemsep2pt
  \item \textbf{Kontakt-Patch-Test, gerade Kanten: maschinengenau.} In 2D (CPE4/CONLINE2,
        ...
        Abweichung der Multiplikatoren von $p$ bei $10^{-15}\ldots10^{-12}$. Das bestätigt
  ...
  \item \textbf{Tangente des eingefrorenen Residuums: exakt.} Test~09 bestätigt
        ...
        ($0{,}27\ldots1{,}17$ von $\max|\bm K|$) -- siehe Abschnitt~\ref{sec:frozen}.
  ...
  \item \textbf{Knotennormalen gegen eine unabhängige Referenz.} Auf einem Zylinderausschnitt
        vom Radius $4$ wird die Knotennormale \eqref{eq:nodalnormal} mit der analytischen
        verglichen, für alle sieben Elementtypen und zwei Netzfeinheiten:
        ...
        & $n=2$ & $n=4$ & Faktor \\
        \midrule
        linear (CONQUAD4, CONTRI3, CONLINE2)      & $7{,}1620^\circ$ & $3{,}5810^\circ$ & $2{,}00$ \\
        quadratisch (CONQUAD8/9, CONTRI6, CONLINE3) & $0{,}0278^\circ$ & $0{,}0035^\circ$ & $7{,}95$ \\
        \bottomrule
        \end{tabular}
        \end{center}

        Geprüft wird nicht bloß, dass der Fehler fällt, sondern eine
        Mindest\emph{fehlerreduktion} bei Halbierung der Elementgröße ($\ge1{,}8$ linear,
        $\ge3{,}5$ quadratisch; die geforderte Reduktion, nicht die Konvergenzordnung -- die
        beiden Schranken entsprechen den Ordnungen $1$ und $2$). Das ist der eigentliche
        Regressionsschutz: eine facettenweise gebildete Normale erfüllt jede blosse Reduktion
        ebenfalls, fällt aber auf Faktor $2{,}00$ bei rund dem $250$-fachen Fehler zurück --
        nachgestellt und als \emph{fehlschlagend} verifiziert.

        Ein zweiter, davon unabhängiger Fall prüft die \emph{Gewichtung} zwischen den
        Nachbarfacetten. Er braucht ein \emph{graduiertes} Netz: auf gleichmäßigen Netzen -- und
        das sind alle übrigen der Reihe -- liefert jede positive Gewichtung dieselbe Richtung.
        Gemessen wird auf demselben Zylinderausschnitt mit Elementbreiten in geometrischer Folge
        (Faktor $2$), und zwar nur an den Knoten, deren Nachbarfacetten sich um mehr als $10\,\%$
        unterscheiden. Diese Einschränkung ist nötig, weil der größte Winkelfehler des Patches am
        \emph{Rand} sitzt, an einem Knoten mit nur einer angrenzenden Facette -- dort gibt es
        nichts zu gewichten, und mit $7{,}64^\circ$ gegenüber $1{,}91^\circ$ im Inneren überdeckt
        er den Effekt vollständig. Die Schranken liegen zwischen den Werten der beiden Varianten
        (Tabelle in Abschnitt~\ref{sec:knotennormale}); die flächengewichtete Summe reißt jede von
        ihnen, ebenfalls nachgestellt und als \emph{fehlschlagend} verifiziert. Geprüft werden
        Werte und nicht eine Rate: bei fester Graduierung bleibt das Größenverhältnis benachbarter
        Facetten unter Verfeinerung konstant und damit auch der Fehleranteil aus der Gewichtung
        (gemessen $1{,}910^\circ \to 1{,}819^\circ$ von $n=4$ auf $n=6$, CONQUAD4).

        Damit die quadratischen Typen wirklich die Formel messen und nicht das Netz, werden die
        Knoten, die der Testaufbau selbst anlegt -- das CONQUAD9-Zentrum und die CONTRI6-Diagonal\-mitte,
        beide als Mittel zweier \emph{Ecken} und damit auf der Sehne -- radial auf den Zylinder
        gezogen (\code{cylinder\_snap}). Ohne das lieferten genau diese beiden Typen weiterhin die
        $7{,}16^\circ$ der linearen Zeile, obwohl die Normalenformel dort längst zweiter Ordnung
        ist. Für alle aus \code{cylinder\_warp} stammenden Knoten ist die Projektion die Identität.

        Zusätzlich wird geprüft, dass keine Normale in den eigenen Körper zeigt, und dass die
        Knoten-Naturkoordinaten zur Knotenreihenfolge der Formfunktionen passen
        ($N_a(\bm\xi_b)=\delta_{ab}$, alle sieben Typen, gemessen exakt $0$).

        Die Referenz ist dabei bewusst \emph{extern}: eine Prüfung der Form
        $\bm n_{j,f}\cdot\partial\bm x/\partial\xi=0$ wäre für die Konstruktion
        \eqref{eq:nodalnormal} eine Identität und damit für jede Eingabe erfüllt.
  \item \textbf{Signorini-Bedingungen, zweifach ausgewertet.} Die Bedingungen $p_n\ge0$,
        ...
        in 3D und in Lastfällen mit Teilkontakt, Abheben und voller Überdeckung; in 2D kommt der
        eigentliche Active-Set-Lastfall hinzu, die verkippte Oberseitenverschiebung
        (\code{schief}), bei der links geschlossene und rechts offene Knoten im \emph{selben}
        Increment auftreten. Beide
        ...
        höchstens $6{,}0\cdot10^{-5}$ der aufgebrachten Verschiebung an Durchdringung, im
...
% ---------------------------------------------------------------------
\subsection{Prüfung in allen drei Formulierungen}
\label{sec:verifformulations}

Seit der Penalty-Formulierung (Abschnitt~\ref{sec:penalty}) steht dieselbe diskrete
Zwangsbedingung auf drei Wegen zur Verfügung. Die Suite läuft deshalb parametrisiert: jeder Test,
der eine Formulierung überhaupt betrifft, wird dreimal ausgeführt --
\code{lagrange}, \code{penalty} und \code{penalty\_uzawa}. Die Tests der reinen Geometrie und
Algebra (\file{01} bis \file{05}) bleiben davon ausgenommen; sie prüfen Größen, die alle drei
teilen, und dreifach ausgeführt prüften sie dreimal dasselbe.

Maßstab ist dabei nicht eine feste Zahl, sondern was die jeweilige Formulierung leisten
\emph{kann}: der Sattelpunkt erfüllt $\tilde g_j=0$ als Gleichung des Systems, das augmentierte
Verfahren bis zu seiner Augmentierungstoleranz, reines Penalty bis auf $O(1/\kappa)$. Schranken an
die Zwangs\emph{erfüllung} werden entsprechend skaliert; Schranken an eine Druckgröße --
etwa $p_n\ge0$ -- bleiben unverändert, denn ein Zugdruck wäre in jeder Formulierung falsch.

\paragraph{Was nicht in jeder Formulierung prüfbar ist.} Drei Prüfungen haben im Penalty-Zweig
kein Gegenstück und melden das ausdrücklich, statt stillschweigend zu bestehen: die
PDASS-Terminierung und die Anti-Cycling-Buchführung (dort wird keine Aktivmenge iteriert,
Abschnitt~\ref{sec:penaltyactiveset}), der NCP-Indikator bei \emph{reinem} Penalty (er prüft
gerade den Zweig, den eine reine Durchdringungsheuristik nicht darstellen kann) und der Boden des
$\lambda$-Residuums (es gibt dort weder Multiplikatorzeile noch Skalarvariablen,
Abschnitt~\ref{sec:scalelimit}).

\paragraph{Ergebnis.} Von $53$ Prüfungen über $31$ Tests bestehen $47$, fünf sind aus den
genannten Gründen ausgenommen, und einer bleibt offen: \file{10\_signorini\_check} unter
\code{penalty\_uzawa} an Knoten mit Splitterüberdeckung
(Abschnitt~\ref{sec:penaltylimits}). Besonders zu erwähnen sind zwei bestandene Prüfungen:

\begin{itemize}\itemsep2pt
  \item \textbf{Die konsistente Tangente} \eqref{eq:penaltytangent} hält der Prüfung gegen
        zentrale Differenzen in allen drei Formulierungen stand, mit
        $1{,}3\cdot10^{-12}$ bis $4{,}2\cdot10^{-11}$ relativ über flache, geneigte und gekrümmte
        Interfaces sowie über beide Zweige der Aktivmenge -- dieselbe Güte wie im
        Multiplikatorzweig. Dass beide Penalty-Varianten dabei \emph{dieselbe} Tangente liefern,
        ist selbst eine Aussage: der augmentierte Schätzer $z_j$ ist während der
        Gleichgewichtsiteration konstant und trägt nichts zur Ableitung bei.
  \item \textbf{Der Kontakt-Patch-Test} wird von \code{penalty\_uzawa} in 2D wie in 3D bestanden,
        von \code{penalty} allein dagegen nur im Rahmen seines $O(1/\kappa)$-Fehlers. Das ist
        keine Schwäche der Umsetzung, sondern genau der Unterschied, dessentwegen
        \citet{puso2008} die äußere Schleife einführen.
\end{itemize}

\paragraph{Zwei-Block-Reihe und Hertz-Kontakt.} Auch die Reihe aus
Abschnitt~\ref{sec:controltests} läuft in allen drei Formulierungen, jede in einem eigenen
Prozess. Alle $138$ Rechnungen ($46\times3$) konvergieren. Von den $46$ Fällen stimmt
\code{penalty\_uzawa} in $40$ mit dem Sattelpunkt überein, und zwar bis auf Löserrauschen:
Kompressionsfamilie $10^{-11}\ldots10^{-9}$, schiefes Interface $1{,}3\cdot10^{-9}$, Hertz
identisch, Abheben in allen drei Formulierungen $\lambda_j=0$. Reines Penalty liegt überall bei
seinem systematischen Fehler von rund $10^{-3}$.

Die sechs verbleibenden Fälle sind die \emph{Gleit}-Varianten, und sie weichen in beiden
Penalty-Formulierungen ab -- der Befund aus Abschnitt~\ref{sec:penaltylimits}. Die Kennzahl ist
dort der Schub, der bei reibungsfreiem Kontakt verschwinden muss; er bleibt im Multiplikatorzweig
bei $5\cdot10^{-15}\ldots10^{-11}$ und erreicht im Penalty-Zweig
$2{,}3\cdot10^{-2}\ldots7{,}0\cdot10^{-2}$. Er ist dabei Folge und nicht Ursache: er entsteht aus
dem ungleichmäßigen Normaldruck an den angeschnittenen Randknoten.
...
$\lambda_j\to0$, $\sigma\to0$, unterer Block in Ruhe. & $\max|\lambda_j|$ & $5{,}4\cdot10^{-12}$ \\[2pt]
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Quadratisch (\code{hex20}/CONQUAD8).} Der Maximaldruck stimmt auf $1{,}0\,\%$ --
        ...
        zu Knoten um $8$--$10\,\%$, und er sinkt nur langsam mit Verfeinerung
        ($10{,}1\to7{,}8\,\%$). Der knotenweise Verlauf sagt genau, wo er herkommt
        (\file{hertz\_profile\_hertz\_hex20\_fine.csv}):

        \begin{center}
        \small
        \begin{tabular}{lrrrrr}
        \toprule
        $x$ & $0{,}0000$ & $0{,}1000$ & $0{,}2000$ & $0{,}3000$ & $0{,}3750$ \\
        \midrule
        $\lambda/p_{\text{Hertz}}$ Eckknoten   & $1{,}0100$ & $1{,}0100$ & $1{,}0102$ & $1{,}0107$ & $1{,}0169$ \\
        $\lambda/p_{\text{Hertz}}$ Mittelknoten & $1{,}0076$ & $1{,}0096$ & $1{,}0222$ & $1{,}0995$ & $1{,}5339$ \\
        \bottomrule
        \end{tabular}
        \end{center}

        Die \emph{Eck}knoten treffen die Hertz-Verteilung über die ganze innere Kontaktzone mit
        konstant $1{,}0100$ -- also bis auf den oben genannten Amplitudenversatz exakt. Der
        gesamte Zickzack sitzt auf den \emph{Mittel}knoten und wächst monoton zum Rand der
        Kontaktzone hin, bis $\lambda$ dort über \emph{beiden} Nachbarecken liegt ($6{,}47$ bei
        $x=0{,}3875$ gegenüber $4{,}96$ und $2{,}27$) -- die Multiplikator-Parabel beult innerhalb
        des Elements nach oben aus.

        Das ist die Antwort des \emph{vollen} quadratischen Multiplikatorraums auf den freien Rand
        der Kontaktzone. Der Hertz-Druck fällt dort wie $\sqrt{a-x}$ auf null, und dieser Rand
        liegt \emph{zwischen} zwei Knoten; eine Parabel je Element kann das nicht darstellen und
        antwortet mit einem Überschwinger am Mittelknoten. Im Inneren, wo der Druck glatt ist,
        gibt es folgerichtig keinen Zickzack.

        \emph{Nicht} die Ursache sind die unterschiedlichen Knotengewichte von Eck- und
        Mittelknoten. $\lambda_j$ ist nach Abschnitt~\ref{sec:sgnd} der \emph{Druck}, nicht die
        Knotenkraft; $\tilde D_{jj}$ skaliert die Kraft. Und da die duale Basis $\bm\Phi=\bm A_e\bm N$
        denselben Raum aufspannt wie $\bm N$, wird ein glattes Druckfeld exakt reproduziert,
        gleichgültig wie ungleich die Gewichte sind. (Frühere Fassungen dieser Doku führten den
        Zickzack auf die Gewichte $\tfrac{7}{18}$ und $\tfrac29$ zurück; das sind zudem die Werte
        des \emph{CONLINE3}, während CONQUAD8 bei $\alpha=\tfrac13$ die Gewichte $\tfrac59$ und
        $\tfrac49$ auf dem Referenzquadrat trägt.)

        Die Literatur kennt für genau diesen Punkt eine Alternative, die hier nicht umgesetzt ist:
        einen Multiplikatorraum \emph{reduzierter} Ordnung, bei dem nur die Eckknoten
        Multiplikatoren tragen. \citet[Abschn.~4]{popp2012} beschreiben ihn als für
        Ungleichungsnebenbedingungen \emph{``even more preferable''}, weil er zusätzlich punktweise
        nichtnegativ ist, und verweisen auf \citet{puso2008}, wo die stückweise linearen
        Multiplikatoren auf quad8-Flächen \emph{``smoother than the linear multiplier results''}
        ausfallen. Das deckt sich mit dem Befund hier: es sind die Mittelknoten-Multiplikatoren,
        die oszillieren.
\end{itemize}
...
Diskretisierungseffekte sind -- Facettengeometrie bei linearen, der Mittelknoten-Überschwinger
des vollen quadratischen Multiplikatorraums am freien Rand der Kontaktzone bei quadratischen
Elementen -- und keine Anzeichen eines Fehlers in der Formulierung. Ein Restpunkt
...
Betonquader nachgerechnet (Versuchsreihe VA1, Einbindetiefe 6\,cm). Der Anker ist ein eigener
Körper, der seine Last ausschließlich über den Kontakt in den Beton einleitet; die Netze beider
Körper sind an der Kontaktfläche nichtkonform. Anders als die Patch- und Referenztests aus
Abschnitt~\ref{sec:verif} ist das Beispiel weder analytisch lösbar noch spannungshomogen.

\subsection{Modell}
Der Beton ist mit gradientenerweiterter Schädigungsplastizität (\code{GCDP}) modelliert, Anker und
Auflager linear-elastisch ($E=210\,000$\,MPa, $\nu=0{,}3$). Das Modell ist ein 4\,mm dicker
Scheibenschnitt; die ausgewertete Kraft wird mit dem Faktor 20 auf die reale Probenbreite von
80\,mm hochgerechnet. Der Ankerkopf wird verschiebungsgesteuert um 1{,}1\,mm nach oben gezogen, bei
Symmetriebedingungen in $x$ und $z$. Die Schrittsteuerung läuft mit
\code{maxInc}${}=10^{-2}$, \code{minInc}${}=10^{-7}$ und höchstens $25$ Newton-Iterationen je
Inkrement.

Der Kontakt zwischen Stahl und Beton ist in \textbf{sechs} getrennte Mortar-Constraints zerlegt
(links und rechts, je Schaftflanke, Kopfunterseite und Kopfflanke), jeweils mit
$c_n=41\,069$; der Beton ist durchgehend die Slave-Seite. Die Zerlegung ist bewusst so gewählt: an
der 90$^\circ$-Kante zwischen Schaft und Kopf wäre eine gemeinsame Kontaktfläche mit mehrdeutiger,
gemittelter Knotennormale behaftet (Abschnitt~\ref{sec:normalen}); getrennte Constraints erhalten
dort die flächeneigene Normale.

Gerechnet wird dasselbe Modell zweimal: einmal linear (\code{GC3D8} für den Beton, \code{C3D8} für
den Stahl, Kontaktelemente \code{CONQUAD4}) und einmal quadratisch (\code{GC3D20R}, \code{C3D20R},
\code{CONQUAD8}). Die beiden Netze sind \emph{geometrisch identisch} -- je $9\,162$ Beton- und
$418$ Stahlelemente, dieselben sechs Kontaktflächen mit je $188$ Facetten, und die Eckknoten
stimmen auf Maschinengenauigkeit überein; das quadratische Netz hat lediglich zusätzliche
Kantenmittelknoten. Daraus folgen $19\,736$ Knoten und $204$ Lagrange-Multiplikatoren im linearen
gegen $68\,496$ Knoten und $498$ Multiplikatoren im quadratischen Fall.
...
Beide Läufe wurden zeitgleich und mit demselben Codestand gerechnet.
...
Kraft-Weg-Kurven der linearen (hex8) und der quadratischen (hex20) Diskretisierung gegen das
Streuband der drei Laborversuche. Beide Rechnungen verwenden dieselbe Gesamt-Auflagerbettung.
Keine der beiden erreicht die vorgegebene Endverschiebung von $1{,}1$\,mm; die Kreuze markieren
das jeweilige Ende. Erzeugt mit \file{POT\_Dejori/plot\_comparison.py}.}
\label{fig:pot}
\end{figure}
...
\begin{tabular}{lcccc}
\toprule
 & Sekantensteifigkeit ($u\le0{,}03$\,mm) & $F_{\max}$ & $u$ bei $F_{\max}$ & $u$ am Ende \\
\midrule
hex8 (\code{GC3D8}/\code{CONQUAD4})    & $199{,}1$\,kN/mm & $22{,}50$\,kN & $0{,}143$\,mm & $0{,}785$\,mm \\
hex20 (\code{GC3D20R}/\code{CONQUAD8}) & $197{,}4$\,kN/mm & $19{,}81$\,kN & $0{,}124$\,mm & $0{,}749$\,mm \\
\midrule
...
\paragraph{Anfangssteifigkeit.}
Die Sekantensteifigkeiten unterscheiden sich um $0{,}9\,\%$; das quadratische, reduziert
integrierte Modell ist das geringfügig weichere. Das deckt sich mit den Referenztests aus
\file{Control\_Tests}, in denen hex8, hex20 und hex20R dieselbe Steifigkeit liefern.

\paragraph{Traglast.}
Am Peak trennen sich die Kurven. Die quadratische Rechnung liegt mit $19{,}81$\,kN an der Oberkante
des Versuchsstreubands ($16{,}63\ldots19{,}53$\,kN) und überschreitet die drei Versuche um $1$,
$19$ und $12\,\%$; die lineare überschätzt sie mit $22{,}50$\,kN um $15$, $35$ und $28\,\%$ und
erreicht den Peak später ($0{,}143$ gegenüber $0{,}124$\,mm).

\paragraph{Entfestigender Ast.}
Nach dem Peak liegt die lineare Rechnung durchgehend über der quadratischen. Auf $81\,\%$ ihres
Nachbruchwegs verläuft sie innerhalb des Streubands, dazwischen -- zwischen $u=0{,}154$ und
$0{,}590$\,mm -- darüber; am Ende steht sie bei $8{,}43$\,kN, also $37{,}5\,\%$ der eigenen
Traglast. Die quadratische Kurve fällt steiler ab: sie unterschreitet das Streuband ab
$u=0{,}200$\,mm und kehrt bis zum Ende der Rechnung nicht mehr in das Band zurück, sodass nur
$6\,\%$ ihres Nachbruchwegs innerhalb liegen; am Ende steht sie bei $3{,}82$\,kN, also
$19{,}3\,\%$ ihrer Traglast.

\subsection{Ende der Rechnung und Rechenaufwand}
Beide Rechnungen enden vor der vorgegebenen Endverschiebung von $1{,}1$\,mm, aus
\emph{unterschiedlichen} Gründen.

\paragraph{hex8: Materialversagen.}
Die lineare Rechnung läuft bis $u=0{,}785$\,mm ($71{,}4\,\%$ des vorgegebenen Wegs), also weit über
den Peak hinaus, und bricht im entfestigenden Ast ab. Der Löser reduziert die Schrittweite bis auf
\code{minInc}${}=10^{-7}$, meldet dort \code{Cannot reduce increment size} und beendet den Step; die
zuletzt gemeldeten Residuenausreißer sitzen im Verschiebungs- und im Schädigungsfeld. Die Kurve
erfasst Anstieg, Traglast und einen großen Teil des Nachbruchbereichs. Aufwand: $199$ Inkremente,
$17$ Cutbacks, $1\,391$ Newton-Iterationen, $1\,725$\,s Rechenzeit (rund $29$ Minuten).

\paragraph{hex20: händischer Abbruch.}
Die quadratische Rechnung wurde nicht vom Löser beendet, sondern nach rund $11$ Stunden von Hand
abgebrochen (Prozess beendet, daher keine Laufzeittabelle im Log; die Zeit ist aus den
Dateizeitstempeln abgeleitet). Sie stand zu diesem Zeitpunkt bei Inkrement $1\,296$ und
$u=0{,}749$\,mm ($68{,}1\,\%$ des vorgegebenen Wegs) und lief konvergent weiter, aber mit
Schrittweiten, bei denen der Restweg nicht mehr in vertretbarer Zeit erreichbar war: die letzten
$50$ Inkremente entsprechen Wegschritten von $6{,}6\cdot10^{-6}$ bis $3{,}2\cdot10^{-5}$\,mm bei
einem Restweg von $0{,}35$\,mm. Aufwand bis dahin: $1\,296$ Inkremente, $63$ Cutbacks, $8\,653$
Newton-Iterationen.

\paragraph{Einordnung des Aufwands.}
Auf dem gemeinsamen Wegstück bis $u=0{,}749$\,mm braucht die quadratische Rechnung $8\,645$
Newton-Iterationen gegenüber $1\,110$ der linearen, also das $7{,}8$-fache, und jede einzelne
Iteration kostet mit rund $4{,}6$ gegenüber $1{,}24$\,s das $3{,}7$-fache. Der zweite Faktor ist
Netzgröße ($3{,}5$-mal so viele Knoten); der erste ist es nicht -- dieselbe Last, dieselbe
Geometrie, dieselben sechs Kontaktpaare, nur mehr Newton-Schritte je Wegstück, und entsprechend
mehr Cutbacks ($63$ gegenüber $17$). Dafür kommen zwei Ursachen in Frage, die diese Rechnung nicht
trennt: die feiner aufgelöste und damit steilere Lokalisierung des Materialmodells, und die
fehlende konsistente Linearisierung des Kontaktbeitrags (Abschnitt~\ref{sec:frozen}), ohne die der
Konvergenzradius kleiner ist. Hier läge der Gewinn einer konsistenten Linearisierung
(Abschnitt~\ref{sec:ausblick:lin}): nicht in der Genauigkeit der Kurve, sondern in der Rechenzeit,
die sie kostet.
...
$\partial\bm n_j/\partial\bm x$ --, und diese Terme sind mit $0{,}27\ldots1{,}17$ von
...
\begin{enumerate}\itemsep2pt
  \item die Knotennormale $\bm n_j$ nach \eqref{eq:nodalnormal} -- die Ableitung einer normierten
        Summe normierter Kreuzprodukte. Für diesen einen Punkt ist die Herleitung bereits
        vorhanden und \emph{wörtlich} übertragbar: \citet[Gl.~(A1)--(A2)]{popp2010} linearisieren
        genau \eqref{eq:nodalnormal} in derselben Gestalt, (A1) die äußere Normierung der Summe
        und (A2) die innere Normierung der einzelnen Facettenbeiträge. Beide Schritte werden
        gebraucht, keiner entfällt;
...
gegenüber $\bm K$ aus. Sie beträgt heute $0{,}27\ldots1{,}17$ und müsste nach Stufe~(IV) auf das
Niveau von Teil~1 fallen, also auf $\sim\!10^{-11}$ (die Bestehensschranke des Tests liegt bei
$10^{-8}$). Solange sie das nicht tut, fehlt ein Term --
...
existiert nicht, und für einen Knoten, der beiden Flächen angehörte, wäre die gemittelte
Knotennormale nach \eqref{eq:nodalnormal} nicht eindeutig definiert.
...
\code{formulation=\path{<str>}} & Optional, Vorgabe \code{lagrange}. Wie die Zwangsbedingung erzwungen wird (Abschnitt~\ref{sec:penalty}): \code{lagrange} über Multiplikatoren als Unbekannte des Gesamtsystems, \code{penalty} über $t_A=\kappa\,g_A$ nach \citet[Gl.~(24)]{puso2004} bzw.\ \citet[Gl.~(7)]{puso2008}. Beide teilen die gesamte geometrische Verarbeitung und sind damit unmittelbar vergleichbar. \\
\code{penaltyStiffness=\path{<float>}} & Optional, nur bei \code{formulation=penalty}. Der Parameter $\kappa$ aus \eqref{eq:penaltylaw}. \textbf{Achtung auf die Einheit:} $\kappa$ multipliziert den \emph{gewichteten} Spalt und ist deshalb nicht der klassische Parameter $\varepsilon_N$ der kontinuierlichen Form \citep[Gl.~(30)/(31)]{wriggers}; es gilt $\kappa=\varepsilon_N/\tilde D_{jj}$. \textbf{Ohne Angabe} wird $\kappa$ nach \eqref{eq:kappadefault} abgeleitet und der Wert zur Laufzeit gemeldet -- eine Regel dieser Implementierung, kein Literaturwert. \\
\code{augmentedLagrange=\path{<bool>}} & Optional, Vorgabe \code{no}. Schaltet die äußere Uzawa-Schleife nach \citet[Gl.~(18)]{puso2008} zu (Abschnitt~\ref{sec:augmented}). Damit erfüllt die Lösung die Zwangsbedingung bis zur Augmentierungstoleranz statt bis auf $O(1/\kappa)$ und hängt nicht mehr von $\kappa$ ab. Setzt eine Gleichgewichtsiteration voraus und steht einem expliziten Löser daher nicht zur Verfügung. Ohne Wirkung bei \code{formulation=lagrange}. \\
\code{augmentationTolerance=\path{<float>}} & Optional, Vorgabe $10^{-6}$. Abbruchschranke der äußeren Schleife als \emph{relative} Änderung des Druckschätzers, $\max_j|z^{(k+1)}_j-z^{(k)}_j|\le\texttt{tol}\cdot\max_j|z^{(k+1)}_j|$ -- die zweite der beiden von \citet{puso2008} genannten Möglichkeiten, gewählt weil sie dimensionslos und damit skaleninvariant ist. \\
\code{maxAugmentations=\path{<int>}} & Optional, Vorgabe $10$. Sicherung gegen eine nicht konvergierende äußere Schleife. Wird die Grenze erreicht, wird das gemeldet: das Increment konvergierte dann auf einem Schätzer, der sich noch bewegt hat. \\
\bottomrule
\end{tabularx}
\caption{Parameter des \code{mortarcontact}-Constraints.}
\end{table}
...
\file{testfiles/mortar\_tests/POT\_Dejori/prepare\_mortar\_mesh.py} -- einem Konverter des
Anwendungsfalls, nicht einem Werkzeug des Rahmenwerks -- gilt:
\begin{itemize}
    \item \textbf{Normalenorientierung:} Elementnormalen müssen aus dem Kontinuumskörper nach außen
          weisen (der Konverter stellt dies über die festen Abaqus-Flächenknotenlisten sicher).
          Der Constraint
...
\bibitem[Yang et al.(2005)]{yangmeng2005} B.~Yang, T.~A.~Laursen, X.~Meng: \emph{Two dimensional mortar contact methods for large deformation frictional sliding}, International Journal for Numerical Methods in Engineering 62 (2005), 1183--1225, \path{doi:10.1002/nme.1222}. (Lokale Kopie: \path{meetings/literature/LaursenMeng_2005_TwoDimensionalMortarContactMethodsForLargeDeformationProblemsWithFriction.pdf} -- der Dateiname weicht vom Titel der Ver\"offentlichung ab.)
