...
festen Set, das damit ein glattes Teilproblem ist. Konkret gilt der Satz als gesetzt, wenn er sich
über \textbf{zwei} aufeinanderfolgende Iterationen nicht geändert hat und mindestens die
\textbf{zweite} Iteration erreicht ist (\file{mortarcontact.py:\allowbreak{}1280}).
...
Änderung der Nachweis eines Zyklus, und das Set wird eingefroren
(\file{mortarcontact.py:\allowbreak{}1270--1300}). Übertragen ist dabei die Idee, einen
...
Prüfbar wird das genau einmal, nämlich sobald das Increment konvergiert ist. Der Constraint wertet
den Indikator \eqref{eq:indicator_code} deshalb zu Beginn des \emph{nächsten} Increments erneut
aus -- mit der Geometrie, die im vorigen in Kraft war, und den Verschiebungen, die dabei
herausgekommen sind (\file{mortarcontact.py:\allowbreak{}595--650}). Reproduziert der eingefrorene
Satz sich dort nicht, so hat das Increment an den betroffenen Knoten den falschen Zweig erzwungen,
also $\tilde g_j=0$ an einem Knoten, der sich öffnen will, oder $\lambda_j=0$ an einem, der
schließen will; seine Lösung erfüllt die Signorini-Bedingungen dann nicht. Das wird gemeldet
(Abschnitt~\ref{sec:implnotes}) und ist über die gesamte Verifikationsreihe nicht eingetreten.
...
die Richtung von $\bm n_I$ und damit die Vorzeichen von Spalt und Druck. Zeigt eine
...
Acht Zustände sind zulässig, aber tragen Voraussetzungen der Formulierung nicht mehr; sie werden
...
Satzes. Der Indikator wird zu Beginn des \emph{nächsten} Increments auf dem konvergierten Zustand
des vorigen neu ausgewertet -- dem einzigen Zeitpunkt, an dem das prüfbar ist. Kippt er dort, so
hat das Increment an diesen Knoten den falschen Zweig erzwungen. \\
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Signorini-Bedingungen, zweifach ausgewertet.} Die Bedingungen $p_n\ge0$,
        ...
        Ausgewertet wird zweimal. Mit den \emph{eingefrorenen} Größen $\bm D,\bm M,\bm n_I$ ist
        ...
        $\bm D,\bm M,\bm n_I$ deshalb an der \emph{konvergierten} Konfiguration neu und misst den
...
\begin{enumerate}\itemsep2pt
  \item die Knotennormale $\bm n_I$ nach \eqref{eq:facetnormal} -- die Ableitung einer normierten
  ...
  \item daraus schließlich $\partial N/\partial\bm x$ und damit
        $\partial\bm D/\partial\bm x$ und $\partial\bm M/\partial\bm x$.
\end{enumerate}
Die vollständige Herleitung findet sich bei \citet{popp2010} und \citet[Kap.~4]{farah}.
...
Linearisierung ist an diesen Übergängen unstetig. Zweitens ist eine konsistent linearisierte
Mortar-Formulierung im Allgemeinen \emph{unsymmetrisch}; \citet{hiermeier2018} weisen das
ausdrücklich aus und untersuchen, welche Terme in den veröffentlichten Formulierungen dafür
vernachlässigt werden. Die heute symmetrische Sattelpunktstruktur (Abschnitt~\ref{sec:loesung}) ginge damit
verloren, was auf die Wahl von Löser und Vorkonditionierer durchschlägt.
...
\begin{enumerate}\itemsep3pt
  \item[\textbf{(I)}] \textbf{Geometrie nachführen, ohne zu linearisieren.} $\bm n_I,\bm D,\bm M$
  ...
  \item[\textbf{(II)}] \textbf{$\partial\bm n_I/\partial\bm x$.} Der in sich geschlossenste Term
...
\bibitem[De Luca et al.(1996)]{deluca1996} T.~De~Luca, F.~Facchinei, C.~Kanzow: \emph{A semismooth equation approach to the solution of nonlinear complementarity problems}, Mathematical Programming 75 (1996), 407--439, \path{doi:10.1007/BF02592192}. (Nicht lokal vorliegend; als Beleg für die Globalisierung semiglatter Newton-Verfahren angeführt.)

\bibitem[Deuflhard(2004)]{deuflhard2004} P.~Deuflhard: \emph{Newton Methods for Nonlinear Problems -- Affine Invariance and Adaptive Algorithms}, Springer Series in Computational Mathematics~35, Springer, Berlin, 2004, ISBN 978-3-540-21099-3. (Nicht lokal vorliegend.)
...
\bibitem[Popp et al.(2010)]{popp2010} A.~Popp, M.~Gitterle, M.~W.~Gee, W.~A.~Wall: \emph{A dual mortar approach for 3D finite deformation contact with consistent linearization}, International Journal for Numerical Methods in Engineering 83 (2010), Heft~11, 1428--1465, \path{doi:10.1002/nme.2866}. (Nicht lokal vorliegend.)
...
\bibitem[Volino \& Magnenat-Thalmann(2000)]{volino2000} P.~Volino, N.~Magnenat-Thalmann: \emph{Virtual Clothing: Theory and Practice}, Springer, Berlin/Heidelberg, 2000, ISBN 3-540-67600-7. (Quelle des Krümmungskriteriums; zitiert nach \citealp{yang2008}, Ref.~[17] -- nicht lokal vorliegend.)
