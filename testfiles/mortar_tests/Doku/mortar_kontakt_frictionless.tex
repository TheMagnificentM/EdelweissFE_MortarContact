...
steht im Code (\file{mortarcontact.py:\allowbreak{}785--795}, Bezeichner \code{C} für $\bm M$).

\medskip
\noindent\textbf{Wann sich die Summe auf einen Term reduziert.} Ist $\bm D$ diagonal, so
vereinfacht sich \eqref{eq:wgap_pos} zu
\begin{equation}
\tilde g_{j} \;=\; \bm n_j\!\cdot\!\Big(\bm D[j,j]\,\bm x_j^{(1)}
- \sum_l \bm M[j,l]\,\bm x_l^{(2)}\Big)
...
sie: dessen Voll-Element-Eckknoten-Integrale sind bereits strikt positiv ($\tfrac19$ auf dem
Referenzquadrat),
...
\begin{quote}\small
\textbf{Einschränkung für CONQUAD9.} Das Argument des vorigen Absatzes trägt nur für das
\emph{Voll-Element}-Integral. Wie in Abschnitt~\ref{sec:alphachoice} ausgeführt, ist in der
segmentbasierten Formulierung aber das Integral über die \emph{tatsächliche Überlappungsfläche}
maßgeblich, und dessen Positivität für beliebige Teilgebiete verlangt \emph{punktweise}
Nichtnegativität \eqref{eq:pointwisepos}. Die Lagrange-Formfunktionen des CONQUAD9 erfüllen das
nicht: die Eck- und Kantenfunktionen wechseln im Element das Vorzeichen. Für CONQUAD9 existiert
also keine Absicherung analog zu $\alpha=\tfrac13$ beim CONQUAD8, und bei Teilüberdeckung können
\emph{negative} Knotengewichte auftreten -- im Verifikationslauf (Test~05, Master um $0{,}3$
verschoben) gemessen bis $-0{,}0109$ bei einem größten Gewicht von $0{,}25$.

Praktisch bleibt das beherrschbar, weil der Code die Vorzeichen konsistent mitführt: mit
$\mathrm{sgn}(\tilde D_{jj})$ (Abschnitt~\ref{sec:ncp}, \eqref{eq:indicator_code}) bleiben
Druckvorzeichen \emph{und} Kraftrichtung auch bei negativem Knotengewicht korrekt
(Abschnitt~\ref{sec:implnotes}). Der gewichtete Spalt verliert an einem solchen Knoten
allerdings seine Deutung als Mittelwert. \textbf{Empfehlung:} für quadratische Flächen
CONQUAD8 verwenden; CONQUAD9 nur bei voller Überdeckung des Slave-Elements.
\end{quote}
...
\subsection{Vorzeichenbehandlung: der \code{sgn\_D}-Guard}
Alle Vorzeichenfragen des Kontakts hängen daran, dass das knotenweise Gewicht
$\tilde D_{jj}=\sum_k D_{jk}$ positiv ist. Das ist der Normalfall
(Abschnitt~\ref{sec:basistrafo}), aber für CONQUAD9 bei Teilüberdeckung nicht garantiert. Der Code
führt deshalb $s_j := \mathrm{sgn}(\tilde D_{jj})$ mit
(\file{mortarcontact.py:\allowbreak{}791--795}) und definiert
\begin{equation}
p_{n,j} = -\lambda_j\,s_j, \qquad g_{\text{sep},j} = \tilde g_j\,s_j ,
\end{equation}
also Druck und Trennspalt so, dass $p_{n,j}\ge0$ stets Druck und $g_{\text{sep},j}>0$ stets einen
offenen Spalt bedeutet. Entscheidend ist, dass damit auch die \emph{Kraftrichtung} korrekt bleibt:
die auf den Slave-Knoten wirkende Normalkraft ist
\begin{equation}
\lambda_j \sum_k D_{jk} = \lambda_j \tilde D_{jj} = -p_{n,j}\,s_j\,\tilde D_{jj}
= -p_{n,j}\,\big|\tilde D_{jj}\big| \;\le\; 0 \quad\text{für } p_{n,j}\ge0,
\end{equation}
also unabhängig vom Vorzeichen von $\tilde D_{jj}$ stets \emph{drückend} (entgegen $\bm n_j$, vom
Master weg). Der Guard hält Druck-, Spalt- und Kraftvorzeichen konsistent; was er \emph{nicht}
leisten kann, ist dem gewichteten Spalt an einem Knoten mit $\tilde D_{jj}<0$ seine Deutung als
Mittelwert zurückzugeben.
